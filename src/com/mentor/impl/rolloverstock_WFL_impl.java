...
import com.mentor.datatable.rolloverstock_WFL_dt;
import com.mentor.resource.ConnectionToDataBase;
import com.mentor.utility.Constants;
import com.mentor.utility.ResourceUtil;


public class rolloverstock_WFL_impl {
	public String getDetails(rolloverstock_WFL_action action){
		...
		String query="";
		try {
			query= " SELECT mobile_number, vch_firm_name, vch_firm_add, int_id, vch_license_type "+
		...
		while(rs.next()){
			...
			action.setApp_id(rs.getInt("int_id"));
	...
	public ArrayList displaylistimpl(rolloverstock_WFL_action act ) {
        ...
        try {
    			...
    			query=  /* " SELECT DISTINCT c.seq,coalesce(g.deo_flg,'NA') as deo_flg,c.int_brand_id,COALESCE(c.final_flag, 'N') as final_flag , b.brand_name,c.int_pack_id, a.package_name,"
    			  ...
    			  + "AND c.int_bwfl_id='"+act.getApp_id()+"'  AND COALESCE(c.dispatch_36,0)<c.int_planned_bottles  and "
    				...
    				"	SELECT DISTINCT c.seq,coalesce(g.deo_flg,'NA') as deo_flg,g.rollover_bottles,g.rollover_boxes,c.int_brand_id,COALESCE(c.final_flag, 'N') as final_flag , b.brand_name,c.int_pack_id, a.package_name,"
    					+ "  c.seq,c.licence_no,c.vch_license_type, c.int_recieved_bottles-coalesce(c.dispatch_36,0) as avlbottle , "
  					+ "   round(((int_recieved_bottles-coalesce(dispatch_36,0))/box_size)+.4) as avlboxes ,a.duty, a.adduty,"
  					+ "      					  ((c.int_planned_bottles-coalesce(c.dispatch_36,0))/c.box_size) as dispatchd_box, c.box_size "
  					+ " FROM distillery.packaging_details_19_20 a  , distillery.brand_registration_19_20 b, bwfl_license.mst_receipt_19_20 c left outer join bwfl_license.rollover_bwfl_stock g on   g.package_id=c.int_pack_id  and g.seq=c.seq  " 
  			 + "  WHERE a.brand_id_fk=b.brand_id and a.vch_renewp='Y'  AND a.brand_id_fk=c.int_brand_id AND a.package_id=c.int_pack_id AND b.brand_id=c.int_brand_id  "
  			 + "  AND c.int_bwfl_id='"+act.getApp_id()+"'  AND COALESCE(c.dispatch_36,0)<c.int_planned_bottles  and "
  			  	+ "	 c.int_recieved_bottles-coalesce(c.dispatch_36,0)>0";
 			...
 			rs=pst.executeQuery();
               ...
               while(rs.next()){
            	   rolloverstock_WFL_dt dt=new rolloverstock_WFL_dt();
            	   dt.setSno(i);
	...
	public String scan(rolloverstock_WFL_action action, int brand_id ,int pack_id) {
		...
		String query = " select distinct b.brand_name ,c.int_pack_id as pckg_id ,a.brand_id_fk as brand_id_fk," +
			 	       " a.package_name as package_name,a.code_generate_through as code_generate_through, " +
			 	       " b.etin_unit_id from  distillery.packaging_details_19_20 a, " +
			 	       " distillery.brand_registration_19_20 b ,bwfl_license.mst_receipt_19_20 c where " +
			 	       " a.brand_id_fk=b.brand_id and a.brand_id_fk=c.int_brand_id and b.brand_id=c.int_brand_id and" +
			 	       " a.brand_id_fk='"+brand_id+"' and  a.package_id =c.int_pack_id and a.package_id='"+pack_id+"' ";
				...
				/*"select a.etin_unit_id as etin_unit_id  ,b.brand_name as brand_name  from licence.fl2_2b_2d_19_20 a , " +
				"distillery.brand_registration_19_20 b where  " +
			        	" b.brand_id=a.int_app_id  and b.brand_id ='"+brand_id+"' ";*/
		
		try {
			System.out.println("query=========="+query);
			System.out.println("brand id="+brand_id);
			System.out.println("pack_id===="+pack_id);
		//	System.out.println("brand id"+dt.getBrand_id());
			con = ConnectionToDataBase.getConnection();
			pstmt = con.prepareStatement(query);
			rs = pstmt.executeQuery();
			if (rs.next()) {
	...
	public void saveCSV(rolloverstock_WFL_action action) throws IOException {
		Connection con = null;
		PreparedStatement stmt = null;
		con = ConnectionToDataBase.getConnection();
		 
		try { 
		String queryList = " delete FROM bwfl_license.rollover_stock_bwfl  where seq="+action.getSeq()+" and  etin='"+action.getEtin_unit_id()+"' and unit_id="+action.getApp_id();
		 stmt = con.prepareStatement(queryList);
		  stmt.executeUpdate();
		  
		String query = " INSERT INTO  bwfl_license.rollover_stock_bwfl(etin, casecode,unit_id,packg_id,seq)VALUES (?, ?,?, ?,"+action.getSeq()+")  ";
	...
	public boolean  etin(String casecode,rolloverstock_WFL_action act) {
		...
		try {
			...
			String query = " SELECT distinct b.code_generate_through FROM distillery.fl2_stock_trxn_19_20 a," +
...
public ArrayList displaylist1impl1(rolloverstock_WFL_action act) {
        ...
        try {
        	...
        	query= " select distinct etin ,casecode from  bwfl_license.rollover_stock_bwfl where seq="+act.getSeq()+" and etin='"+act.getEtin_unit_id()+"' and unit_id="+act.getApp_id();
 			...
 			rs=pst.executeQuery();
               ...
               while(rs.next()){
            	   rolloverstock_WFL_dt dt1=new rolloverstock_WFL_dt();
            	   dt1.setSrno(i);
...
public String saveData(rolloverstock_WFL_action action) {
	Connection conn = null;
	PreparedStatement pstmt = null;
	ResultSet rs = null;
	int saveStatus = 0;
	...
	try { 
		...
		String query =" INSERT INTO bwfl_license.rollover_bwfl_stock " +
			 		   ...
			 		   "  '"+action.getRolloverbottles()+"',  '"+action.getSize()+"', '"+action.getRolloverbox()+"',  '"+action.getEtin_unit_id()+"',  "+action.getSeq()+")";
						
 pstmt = conn.prepareStatement(query);
 
 saveStatus=pstmt.executeUpdate();
			if (saveStatus > 0) {saveStatus=0;
				  String query1 = " update bwfl_license.mst_receipt_19_20  set  final_flag='T' where int_bwfl_id='"+ action.getApp_id()+"'  " +
				 		          " and  seq="+action.getSeq()+" and  int_brand_id='"+action.getBrand_id()+"'" +
				 				" and int_pack_id='"+action.getPackage_id()+"' and box_size='"+action.getSize()+"'";
...
public void resetcsvcasecode(rolloverstock_WFL_action action) {
	...
	//	SELECT etin, casecode FROM bwfl.rollover_stock where  etin='"+act.getEtin_unit_id()+"'
			queryList = " delete FROM bwfl_license.rollover_stock_bwfl  where  seq="+action.getSeq()+" and etin='"+action.getEtin_unit_id()+"' and unit_id="+action.getApp_id();
...
public int displaylist1implcount(rolloverstock_WFL_action act) {
        ...
        try {
        	...
        	query= " select distinct count(*) from  bwfl_license.rollover_stock_bwfl where seq="+act.getSeq()+"  and etin='"+act.getEtin_unit_id()+"' and unit_id="+act.getApp_id();
...
public String finalize(rolloverstock_WFL_action act,rolloverstock_WFL_dt dt) {
	Connection conn = null;
	PreparedStatement pstmt = null;
	ResultSet rs = null;
	int saveStatus = 0;
	...
	try {
		...
		conn.setAutoCommit(false);
			...
			String query = " update  bwfl_license.rollover_bwfl_stock  set  deo_flg ='T' where " +
