...
public class ImportPermitImpl {

	public void getDetails(ImportPermitAction act) {
		...
		try {
			...
			String selQr = "select vch_license_number,vch_license_type,mobile_number,stateid,b.vch_state_name,vch_firm_name, "
					...
					+ " license_issue_date,approval_dt,application_date from bwfl.registration_of_bwfl_lic_holder_19_20 a,public.state_ind b,public.district c,public.other_unit_registration d "
	...
	public ArrayList getBrandName() {
		...
		ImportPermitAction act = (ImportPermitAction) facesContext
				.getApplication().createValueBinding("#{importPermitAction}")
		...
		try {
			...
			System.out.println("Brand list=" + sql);
			con = ConnectionToDataBase.getConnection();
	...
	public String getqty(String brand_Id, String packging_Id) {
		...
		try {
			...
			System.out.println("qty=" + queryList);
			con = ConnectionToDataBase.getConnection();
	...
	public String getEtinNmbr(String brand_Id, String packging_Id,
		...
		try {
			...
			String queryList = " SELECT a.brand_id, a.brand_name ,b.quantity, b.package_name ,b.package_id, b.code_generate_through,b.duty,b.adduty "
					...
					+ " WHERE a.brand_id=b.brand_id_fk  "
					+ " AND brand_id =?  AND b.package_id=?";
			System.out.println(brand_Id + " | " + packging_Id + " | "
					+ queryList);
			con = ConnectionToDataBase.getConnection();
	...
	public int maxId(int district_id) {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String query = " SELECT max(id) as id FROM bwfl_license.import_permit where district_id='"
	...
	public int app_id() {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String query = " SELECT max(app_id) as app_id FROM bwfl_license.import_permit";
		int maxid = 0;
		try {
			con = ConnectionToDataBase.getConnection();
			pstmt = con.prepareStatement(query);
	...
	public boolean save(ImportPermitAction act) {
		...
		try {
			...
			query = "INSERT INTO bwfl_license.import_permit_dtl(fk_id, district_id, brand_id, pckg_id, etin, no_of_cases, no_of_bottle_per_case,"
			...
			if (act.getDisplaylist().size() > 0) {
				for (int i = 0; i < act.getDisplaylist().size(); i++) {
					ImportPermitDT dt = (ImportPermitDT) act.getDisplaylist()
							.get(i);
			...
			if (saveStatus > 0) {
				query = "INSERT INTO bwfl_license.import_permit( id, district_id, bwfl_id, unit_id, bwfl_type, import_fee,"
...
return flg;
	...
	public ArrayList hislistImpl(ImportPermitAction act) {
		...
		try {
			selQr = "select COALESCE(print_permit_pdf,'NUL')print_permit_pdf,COALESCE(digital_sign_pdf_name,'NUL')digital_sign_pdf_name,app_id,import_fee_challan_no,spcl_fee_challan_no,id,district_id,bwfl_id,unit_id,bwfl_type,import_fee,duty+add_duty as duty,special_fee,cr_date,"
					...
					+ ",COALESCE(vch_status,'Pending')as vch_status, vch_approved,valid_upto from bwfl_license.import_permit where district_id='"
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				ImportPermitDT dt = new ImportPermitDT();
	...
	public ArrayList viewdetailImpl(ImportPermitAction act, int app_id,
			int district_id) {
		ArrayList list = new ArrayList();
		Connection conn = null;
		PreparedStatement ps = null;
		...
		try {
			selQr = "select a.brand_id,b.brand_name,a.pckg_id,c.package_name,a.etin,a.no_of_cases,a.no_of_bottle_per_case, "
					...
					+ "FROM bwfl_license.import_permit_dtl a,distillery.brand_registration_19_20 b,distillery.packaging_details_19_20 c "
			...
			while (rs.next()) {
				 ...
				 * pstmt.setInt(3, Integer.parseInt(dt.getBrand_id()));
				 * pstmt.setInt(4, Integer.parseInt(dt.getPckg_id()));
				 * pstmt.setString(5, dt.getEtin()); pstmt.setInt(6,
				 * dt.getNo_of_cases()); pstmt.setLong(7,
				 * dt.getNo_of_bottle_per_case()); pstmt.setInt(8,
				 * dt.getPland_no_of_bottles()); pstmt.setDouble(9,
				 * dt.getCal_importFee()); pstmt.setDouble(10,
				 * dt.getCalculated_duty()); pstmt.setDouble(11,
				 * dt.getCalculated_add_duty()); pstmt.setDouble(12, 2.2);
				 */
				list.add(dt);
				// i++;
			}
	...
	public void getBalance(ImportPermitAction act) {
		...
		try {
			...
			/*
			 * selQr =
			 * " SELECT x.bwfl_id, SUM(x.challan_duty-(x.ap_duty))as bal_duty,                                        "
			 * +
			 * " SUM(x.challan_import_fee-(x.ap_import_fee))as bal_import_fees,                                 "
			 * +
			 * " SUM(x.challan_special_fee-(x.ap_special_fee))as bal_special_fees                              "
			 * +
			 * " FROM                                                                                                    "
			 * +
			 * " (SELECT a.int_distillery_id as bwfl_id, 0 as ap_import_fee, 0 as ap_duty, 0 as ap_special_fee,  "
			 * + " CASE  " +
			 ...
			 * +
			 * " WHEN b.vch_head_code='003900103010000' and 'BWFL2D'=a.vch_bwfl_shop_type THEN SUM(b.double_amt) end as challan_duty, "
			 * +
			 ...
			 * + " AND a.int_distillery_id='"+act.getInt_id()+
			 * "'                                                                              "
			 * +
			 ...
			 * +
			 * " SUM(a.special_fee) as ap_special_fee, 0 as challan_duty, 0 as challan_import_fee, 0 as challan_special_fee                                    "
			 * + " FROM bwfl_license.import_permit a    " +
			 * " WHERE a.bwfl_id='"+act.getInt_id()+
			...
			System.out.println("balance BWFL =" + selQr);
			pstmt = con.prepareStatement(selQr);
			rs = pstmt.executeQuery();
			while (rs.next()) {
	...
	public int delete(ImportPermitAction action, int id, int dist) {
		...
		sql = "delete from bwfl_license.import_permit  where id='" + id
		...
		try {
			...
			if (saveStatus > 0) {
				saveStatus = 0;
				sql = "delete from bwfl_license.import_permit_dtl where fk_id='"
						+ id
						+ "' and district_id='"
						+ dist
						+ "' and login_type='BWFL'";
	...
	public double getBondPermitSpecialFee(ImportPermitDT dt) {
		FacesContext facesContext = FacesContext.getCurrentInstance();
		ImportPermitAction ac = (ImportPermitAction) facesContext
				.getApplication().createValueBinding("#{importPermitAction}")
				.getValue(facesContext);
	...
	public void ChallanList(ImportPermitAction act) {
			...
			con = ConnectionToDataBase.getConnection();
			 selQr = " select chalan_no,chalan_date,amount from bwfl_license.chalan_deposit_bwfl_fl2d where chalan_no in  ( SELECT DISTINCT "
				...
				//	+ " ,CASE WHEN head_type like 'Special%' then chalan_no else '' end as spcl_fee_challan_no "
					...
					" from bwfl_license.import_permit  " +
	...
	public boolean getChallanBalance(ImportPermitAction act) {
