...
import com.mentor.action.GroupingOfUnmappedAction_20_21;
...
public class GroupingOfUnmappedImpl_20_21 {
	// =====================get details of distillery======================

		public int getId(GroupingOfUnmappedAction_20_21 act) {
		...
		public ArrayList getData(GroupingOfUnmappedAction_20_21 act)
		{
			...
			String sql = 	" SELECT c.export_box_size,c.quantity,c.code_generate_through,coalesce(b.tracking_flg,'N') as tracking_flg, "+
							...
							" and b.int_bwfl_id =(select unit_id from bwfl.registration_of_bwfl_lic_holder_20_21 where int_id=?) AND a.maped_unmaped_type='U' AND a.group_id IS NULL and  a.int_distillery_id='" + licno+"'"+
			...
			try {
				...
				try {
					ps.setInt(1, Integer.parseInt(act.getSelect_lic_no()));
				} catch (Exception e) {
					ps.setInt(1, 0);
				}
		...
		public void save(GroupingOfUnmappedAction_20_21 act, ArrayList list)
		{
			...
			try{
				...
				if (list.size() > 0) {
					...
					for (int i = 0; i < list.size(); i++) {
						...
						updtQr = 	" UPDATE bwfl_license.mst_bottling_plan_20_21 SET " +
		...
		public ArrayList getSavedData(GroupingOfUnmappedAction_20_21 act)
		{
		...
		public boolean finalizeData(GroupingOfUnmappedDT dt)
		{
			...
			String sql=
				...
				"	UPDATE bwfl_license.mst_bottling_plan_20_21 "+
