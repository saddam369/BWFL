...
import com.mentor.action.ImportChallanForPermitBWFL_FL2DAction_20_21;
...
public class ImportChallanForPermitBWFL_FL2DImpl_20_21 {

	
	public boolean getDetails(ImportChallanForPermitBWFL_FL2DAction_20_21 act) {
		...
		String selQr="select int_id as app_id,vch_firm_district_id::numeric as district_id,unit_id, 'BWFL' as login_type,vch_license_type from " +
					...
					" licence.fl2_2b_2d_19_20 where vch_license_type='FL2D' and vch_mobile_no='"+ ResourceUtil.getUserNameAllReq().trim() + "' ";
	...
	public ArrayList datalist(ImportChallanForPermitBWFL_FL2DAction_20_21 act) {
	...
	public void ChallanList(ImportChallanForPermitBWFL_FL2DAction_20_21 act) {
		System.out.println("-/-");
		...
		try {
			...
			con = ConnectionToDataBase.getConnection();
			 selQr = " select chalan_no,chalan_date,amount from bwfl_license.chalan_deposit_bwfl_fl2d where chalan_no in  ( SELECT DISTINCT "
					...
					" EXCEPT  " +
					" select DISTINCT import_fee_challan_no  from bwfl_license.import_permit_20_21 union  select DISTINCT import_fee_challan_no  from bwfl_license.import_permit " +
			...
			act.setImportChallanList(ImportChallanList);
			 selQr = " select chalan_no,chalan_date,amount from bwfl_license.chalan_deposit_bwfl_fl2d where chalan_no in  ( SELECT DISTINCT "+
			 		" CASE WHEN head_type like 'Special%' then chalan_no else '' end as spcl_fee_challan_no "
						...
						+ " CASE WHEN 1="+act.getVch_license_type()+" THEN 'BWFL2A' WHEN 2="+act.getVch_license_type()+" THEN 'BWFL2B' WHEN 3="+act.getVch_license_type()+" THEN 'BWFL2C' WHEN 4="+act.getVch_license_type()+" THEN 'BWFL2D' END " +
						" EXCEPT  " +
						" select spcl_fee_challan_no  from bwfl_license.import_permit_20_21  union select spcl_fee_challan_no  from bwfl_license.import_permit" +
						"   ) and  unit_id="+act.getInt_id()+" AND unit_type= "
						+ " CASE WHEN 1="+act.getVch_license_type()+" THEN 'BWFL2A' WHEN 2="+act.getVch_license_type()+" THEN 'BWFL2B' WHEN 3="+act.getVch_license_type()+" THEN 'BWFL2C' WHEN 4="+act.getVch_license_type()+" THEN 'BWFL2D' END " ;
	...
	public void ChallanList1(ImportChallanForPermitBWFL_FL2DAction_20_21 act) {
		System.out.println("-/-");
		ArrayList ImportChallanList = new ArrayList();
		ArrayList SpclChallanList = new ArrayList();
		Connection con = null;
...
public boolean getChallanBalance(ImportChallanForPermitBWFL_FL2DAction_20_21 act,double import_fee,double special_fee) {
...
public boolean getChallanBalanceBWFL(ImportChallanForPermitBWFL_FL2DAction_20_21 act,double import_fee,double special_fee) {
...
System.out.println("getChallanImportFee-"+sql);
	...
	public boolean update(ImportChallanForPermitBWFL_FL2DAction_20_21 act){
	...
	//String navigation="NA";
		...
		if(act.getDatalist().size()>0){
			...
			ImportChallanForPermitBWFL_FL2DDT_20_21 dt = (ImportChallanForPermitBWFL_FL2DDT_20_21) act.getDatalist().get(i);
