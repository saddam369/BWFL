...
import com.mentor.action.Generate_BarCode_QRCode_For_CSD_Action;
import com.mentor.action.Generate_BarCode_QRCode_For_CSD_Action;
import com.mentor.datatable.Generate_Bar_Code_QR_Code_For_CSD_DT;
import com.mentor.datatable.Generate_Bar_Code_QR_Code_For_CSD_DT;
...
public class Generate_BarCode_QRCode_For_CSD_Impl {

	public boolean getSugarmill(Generate_BarCode_QRCode_For_CSD_Action ac) {
		int id = 0;
		boolean flag=false;
		Connection con = null;
		...
		try {
			...
			String queryList = 	" SELECT 'O' as unit_type, int_app_id_f FROM public.other_unit_registration_20_21 where login_id" +
								" = '"+ ResourceUtil.getUserNameAllReq().trim() + "' and vch_indus_type like '%CSD%' " +
								" union SELECT 'D' as unit_type, int_app_id_f FROM public.dis_mst_pd1_pd2_lic where vch_wrk_phon::text" +
					            " = '"+ ResourceUtil.getUserNameAllReq().trim() + "' " +
					            " union SELECT 'B' as unit_type,vch_app_id_f FROM public.bre_mst_b1_lic where vch_reg_mobile" +
					            " = '"+ ResourceUtil.getUserNameAllReq().trim() + "'" +
				               	" union SELECT 'IU' as unit_type,int_app_id_f FROM public.other_unit_registration_20_21 where login_id" +
					            " = '"+ ResourceUtil.getUserNameAllReq().trim() + "' and vch_indus_type='IU' and int_app_id_f in (select distinct int_fl2d_id from distillery.brand_registration_20_21 "+
			...
			if (rs.next()) {
				ac.setCsd_id(rs.getInt("int_app_id_f"));
                ac.setUnit_type(rs.getString("unit_type"))  ;
                flag=true;
			}
	...
	public int getSugarmill_Id() {
		...
		try {
			...
			String queryList = " select int_app_id, vch_firm_name  from licence.fl2_2b_2d_20_21 where vch_license_type='FL2D' and vch_mobile_no = '"
					+ ResourceUtil.getUserNameAllReq().trim() + "' ";
	...
	public ArrayList getLicenseNo(Generate_BarCode_QRCode_For_CSD_Action action, String lice) {
		ArrayList list = new ArrayList();
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		String SQl = " select distinct license_number FROM distillery.brand_registration_20_21 where " +
...
//////System.out.println("get license="+SQl);
	...
	public ArrayList getBrandName() {
		...
		Generate_BarCode_QRCode_For_CSD_Action bof = (Generate_BarCode_QRCode_For_CSD_Action) facesContext
				.getApplication()
		...
		// sql=
		// "SELECT brand_id, brand_name FROM distillery.brand_registration_20_21 where distillery_id=? order by brand_id ";
		...
		try {
			...
			/*if (lic.equalsIgnoreCase("CL")) {
				...
				sql="SELECT a.brand_id, a.brand_name FROM distillery.brand_registration_20_21 a,fl2d.fl2d_unit_mapping b" +
	...
	public String getqty(String brand_Id, String packging_Id,Generate_Bar_Code_QR_Code_For_CSD_DT dt) {
		String qty = "";
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public int getMax_id() {
		...
		String query = "select nextval('fl2d.mst_stock_receive_sequence_20_21')as id";
	...
	public int maxDutyRegId() {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String query = " SELECT max(int_id) as id FROM distillery.duty_register_20_21 ";
		int maxid = 0;
	...
	public void save(Generate_BarCode_QRCode_For_CSD_Action action) {
		...
		try {
			...
			String query =  " INSERT INTO fl2d.mst_stock_receive_20_21(int_fl2d_id, int_brand_id, " +
			...
			insDutyQr = " INSERT INTO distillery.duty_register_20_21( " +
			...
			if (action.getBrandPackagingDataList().size() > 0) {
				...
				for (int i = 0; i < action.getBrandPackagingDataList().size(); i++) {
					...
					int j = 1;
					Generate_Bar_Code_QR_Code_For_CSD_DT table = (Generate_Bar_Code_QR_Code_For_CSD_DT) action.getBrandPackagingDataList().get(i);
	...
	public void save1(Generate_BarCode_QRCode_For_CSD_Action action) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		int saveStatus = 0;
		try {
			String query = " INSERT INTO distillery.mst_bottling_plan_20_21(int_distillery_id, " +
	...
	public ArrayList getAddRowData(Generate_BarCode_QRCode_For_CSD_Action ac, String lic_typ) {
		ArrayList list = new ArrayList();
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		String sql = "	SELECT int_fl2d_id, int_brand_id, int_pack_id, int_quantity,  "
				...
				+ "		FROM fl2d.mst_stock_receive_20_21 where  int_fl2d_id=? and vch_license_type=?  "
		...
		try {
			...
			while (rs.next()) {
				...
				Generate_Bar_Code_QR_Code_For_CSD_DT dt = new Generate_Bar_Code_QR_Code_For_CSD_DT();
	...
	public ArrayList getData(Generate_BarCode_QRCode_For_CSD_Action ac) {
		ArrayList list = new ArrayList();
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		/* sql ="SELECT a.seq,c.code_generate_through,coalesce(b.tracking_flg,'N') as tracking_flg,a.finalized_date,a.finalized_flag,a.int_fl2d_id,a.licence_no,replace(a.licence_no,' ','')as licencenoo ,a.int_brand_id,b.brand_name, a.int_pack_id,c.package_name , a.int_quantity, "
				...
				+ "		FROM fl2d.mst_stock_receive_20_21 a ,distillery.brand_registration_20_21 b, "
...
/*@rvind*/	" ,e.vch_firm_name||' ('|| e.vch_mobile_no||')' as unit_name                                                            "+
			" FROM distillery.brand_registration_20_21 b,distillery.packaging_details_20_21 c , " +
			" distillery.imfl_type d,fl2d.mst_stock_receive_20_21 a    "+
/*@rvind*/	" left join licence.fl2_2b_2d_20_21 e on  e.vch_license_type='FL2D' and e.int_app_id=a.int_fl2d_id                                  "+
...
//////System.out.println("data list="+sql);
	...
	public boolean ckeck(Generate_BarCode_QRCode_For_CSD_Action ac) {
	...
	public String getBrandPackagingGtinNo(Generate_BarCode_QRCode_For_CSD_Action action,
			Generate_Bar_Code_QR_Code_For_CSD_DT dt) {
	...
	public int getBrandPackagingNoOfBottle(Generate_BarCode_QRCode_For_CSD_Action action,
			Generate_Bar_Code_QR_Code_For_CSD_DT dt) {
	...
	public boolean write(Generate_Bar_Code_QR_Code_For_CSD_DT dt,
			Generate_BarCode_QRCode_For_CSD_Action action, Connection conn) {
		...
		try {
				...
				System.out.println("Date== "+ Utility.convertUtilDateToSQLDate(dt
						.getShowDataTable_Date())+" gtin==="+dt.getGtinno()+"Req Id=="+dt.getRequest_id());
	...
	public void dataFinalize(Generate_BarCode_QRCode_For_CSD_Action action,
			Generate_Bar_Code_QR_Code_For_CSD_DT dt) {
		Connection conn = null;
		...
		String update = "UPDATE fl2d.entry_of_permit_master_detail_20_21 "
			...
			if (!gtinNo.equals("") && serialno != 0) {
				...
				status = pstmt1.executeUpdate();
					...
					//	pstmt1.setInt(1, action.getDistillery_id());
						...
						pstmt1.setInt(7, action.getCsd_id());
	...
	public void generateReport(Generate_Bar_Code_QR_Code_For_CSD_DT dt,
			Generate_BarCode_QRCode_For_CSD_Action action) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String relativePath = Constants.JBOSS_SERVER_PATH
	...
	public String getMinMaxSerialNo(Generate_Bar_Code_QR_Code_For_CSD_DT dt,
			Generate_BarCode_QRCode_For_CSD_Action action) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public String getMinMaxSerialNoForExcel(Generate_Bar_Code_QR_Code_For_CSD_DT dt,
			Generate_BarCode_QRCode_For_CSD_Action action, Connection conn) {
	...
	public boolean checkData(Generate_BarCode_QRCode_For_CSD_Action action) {
		boolean flag = false;
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			if (action.getBrandPackagingDataList().size() > 0) {
				...
				pstmt = conn.prepareStatement(sql);
				for (int i = 0; i < action.getBrandPackagingDataList().size(); i++) {
					Generate_Bar_Code_QR_Code_For_CSD_DT table = (Generate_Bar_Code_QR_Code_For_CSD_DT) action
	...
	public ArrayList getUnit() {
		...
		String SQl = " SELECT int_app_id, vch_applicant_name, vch_firm_name ,"
				...
				+ " FROM licence.fl2_2b_2d_20_21 ,public.district b  WHERE vch_license_type= 'FL2D' and " +
	...
	public void update(Generate_BarCode_QRCode_For_CSD_Action ac) {
	...
	String query=	" UPDATE fl2d.mst_stock_receive_20_21 SET "+
	...
	try { 
			...
			Generate_Bar_Code_QR_Code_For_CSD_DT table = (Generate_Bar_Code_QR_Code_For_CSD_DT) ac.getBrandPackagingDataList().get(i);
			...
			if(saveStatus>0){
				...
				saveStatus=0;
				String sql="update distillery.duty_register_20_21 set int_quantity='"+totalBottles+"',int_value='"+totalPermitFees+"' where int_id='"+ac.getDutyRegisterId()+"'";
	...
	public ArrayList getData1(Generate_BarCode_QRCode_For_CSD_Action ac) {
		ArrayList list = new ArrayList();
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		if(ac.getUnit_type().equalsIgnoreCase("O")){
			filter=" and e.int_app_id_f=c.int_fl2a_id and e.int_app_id_f=f.unit_id and f.unit_type='O' and f.unit_id= "+ac.getCsd_id()+"";
		    filter1="public.other_unit_registration_20_21 e";
		}
		else if(ac.getUnit_type().equalsIgnoreCase("D")){
			filter=" and e.int_app_id_f=c.distillery_id and e.int_app_id_f=f.unit_id and f.unit_type='D' and f.unit_id= "+ac.getCsd_id()+"";
		    filter1="public.dis_mst_pd1_pd2_lic e";	
		}
				
		else if(ac.getUnit_type().equalsIgnoreCase("B")){
			filter=" and e.vch_app_id_f=c.brewery_id and e.vch_app_id_f=f.unit_id and f.unit_type='B' and f.unit_id= "+ac.getCsd_id()+"";
		    filter1="public.bre_mst_b1_lic e";
		}
		
		else if(ac.getUnit_type().equalsIgnoreCase("IU")){
			
			filter=" and e.int_app_id_f=f.unit_id and f.unit_type='IU' and  e.vch_indus_type='IU' and  f.unit_id= "+ac.getCsd_id()+"";
			filter1="public.other_unit_registration_20_21 e";
			
			//filter=" and e.int_id=f.unit_id and f.unit_type='IU' and  f.unit_id= "+ac.getCsd_id()+"";
		  //  filter1="custom_bonds.custom_bonds_master e";
		}

		
		
		sql = 	" SELECT distinct a.seq,COALESCE(a.permit_no, 'NA') as permit_no,b.size,d.code_generate_through,f.permit_nmbr,f.digital_sign_date,b.int_id," +
				" c.liquor_type,d.package_name,c.brand_name,c.vch_license_type,coalesce(c.tracking_flg,'N') as tracking_flg,a.issue_date," +
				" b.brand_id,a.csd_name, COALESCE" +
				" (b.finalize_flg,'NA') as finalize_flg,b.finalized_date,b.packaging_id, a.licence_id, b.no_of_bottle,  "+
				"	b.no_cases,                                                                                                                                                                                            "+
				"	b.no_cases FROM fl2d.entry_of_permit_master_20_21 a, fl2d.entry_of_permit_master_detail_20_21 b,                                                                                                                   "+
				"	distillery.brand_registration_20_21 c ,distillery.packaging_details_20_21 d,"+filter1+"," +
				"  fl2d.application_csd_permit_mst_20_21  f WHERE c.brand_id=d.brand_id_fk and d.package_id=b.packaging_id  and   "+
				"	a.seq=b.seq and c.brand_id=b.brand_id  and a.permit_no=f.permit_nmbr                                                                                                                            "+
				"	"+filter+" and f.digital_sign_date is not null                                                                                                                 "+
		...
		/*sql=" SELECT a.cancel_order_no,a.cancel_req_reason,a.cancel_req_flg,COALESCE(a.cancel_req_dt_time,'0')cancel_req_dt_time,a.seqfrom,a.caseseq,a.seq,c.code_generate_through,coalesce(b.tracking_flg,'N') as tracking_flg,a.finalized_date,      				  "+
			...
			" a.int_liquor_type,d.description, a.vch_license_type, a.plan_dt ,a.permit_no                                             "+
@rvind	" ,e.vch_firm_name||' ('|| e.vch_mobile_no||')' as unit_name                                                            "+
			" FROM distillery.brand_registration_20_21 b,distillery.packaging_details_20_21 c ,distillery.imfl_type d,fl2d.mst_stock_receive_20_21 a    "+
@rvind	" left join licence.fl2_2b_2d_20_21 e on  e.vch_license_type='FL2D' and e.int_app_id=a.int_fl2d_id                                  "+
			" where a.int_brand_id=b.brand_id and b.brand_id=c.brand_id_fk and a.int_pack_id=c.package_id and a.int_liquor_type=d.id  "+
			" and a.int_fl2d_id =? ORDER BY a.plan_dt DESC  ";*/
		
		 System.out.println("sel query=="+sql);
		try {
			con = ConnectionToDataBase.getConnection();
			ps = con.prepareStatement(sql);
		 	//ps.setInt(1, ac.getDistillery_id());
	...
	/*			if(rs.getString("cancel_req_reason")!=null)
				...
				dt.setCancel_flg(rs.getString("cancel_req_flg"));*/
               dt.setFinalize_Date(Utility.convertSqlDateToUtilDate(rs.getDate("finalized_date")));
                if(rs.getString("finalize_flg").equalsIgnoreCase("F"))
                {
				dt.setCancel_req_dt_time("1");
                }else
                	dt.setCancel_req_dt_time("0");
				//dt.setLiqureTypeId(rs.getString("login_type"));
				dt.setRequest_id(rs.getInt("int_id"));
				//dt.setGtinno(rs.getString("code_generate_through"));
				dt.setShowDataTable_Date(rs.getDate("issue_date"));
				dt.setShowDataTable_LiqureType(rs.getString("liquor_type"));
				dt.setShowDataTable_LicenceType(rs.getString("vch_license_type"));
				dt.setShowDataTable_Brand(rs.getString("brand_name"));
				dt.setShowDataTable_Packging(rs.getString("package_name"));
				//dt.setNewml(rs.getInt("quantity"));
				dt.setNewsize(new Double((Long.parseLong(rs.getString("no_of_bottle"))/Long.parseLong(rs.getString("no_cases")))).intValue());
				...
				dt.setPackagingId(rs.getInt("packaging_id"));
				//dt.setLicenceNo(rs.getString("lic_no"));
				////System.out.println("dt.setLicenceNo="+dt.getLicenceNo());
				
                     
				dt.setShowDataTable_PermitNo(rs.getString("permit_nmbr").replaceAll("/", ""));
				
			    
				dt.setLicencenoo(rs.getString("permit_nmbr").replaceAll("/", ""));
				...
				dt.setUnit_id(rs.getInt("licence_id"));
				dt.setUnit_name(rs.getString("csd_name"));
				//dt.setSeqfrm(rs.getLong("seqfrom"));
				//dt.setCaseseq(rs.getLong("caseseq"));
				list.add(dt);
				
				//System.out.println("flag==="+dt.isPdf_flag());
			}
	...
	public boolean checkpermit(String permitno_entered, Generate_BarCode_QRCode_For_CSD_Action action)
	{
////System.out.println("55");
		...
		String SQl = 	" SELECT  * FROM fl2d.mst_stock_receive_20_21  " +
	...
	/*public boolean fl2dUnitMappingId(String permitno_entered, Generate_BarCode_QRCode_For_CSD_Action action)
	{
////System.out.println("55");
		boolean flag = false;
		Connection con = null;
	...
	public int getDutyRegisterId(Generate_BarCode_QRCode_For_CSD_Action ac, String DutyRegisterId) {
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		int dutyRegisterId=0;
		String SQl ="select int_id,int_quantity, int_value from distillery.duty_register_20_21 where permitno_fl2d='"+DutyRegisterId+"'";
	...
	*/	////System.out.println("get other unit list="+sql);
	...
	//================
	public String getLicNoUnitId(Generate_BarCode_QRCode_For_CSD_Action ac, String id) {
		...
		try {
			...
			String queryList = 	"select vch_licence_no from licence.fl2_2b_2d_20_21 where  vch_license_type='FL2D' and int_app_id='"+id+"'  ";
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void cancelPermit(Generate_Bar_Code_QR_Code_For_CSD_DT dt,Generate_BarCode_QRCode_For_CSD_Action action) {
		Connection conn1 = null;
		Connection conn = null;
		PreparedStatement pstmt1 = null; 
		PreparedStatement pstmt = null; 
		int status = 0;
		String update="UPDATE fl2d.mst_stock_receive_20_21 SET cancel_order_no=? ,cancel_order_date=?,cancel_authority_name=?,cancel_order_date_time=?,cancel_pdf_name=?" +
				" WHERE int_fl2d_id=? and int_brand_id=? and int_pack_id=? and plan_dt=? and permit_no=? ";
		
	String bottling2="UPDATE bottling_unmapped.fl2d_20_21  "+
		"SET  cancel_permit=? "+
		"WHERE plan_id=? and  date_plan=? and  etin=? ";
	
	
	String bottling="delete from  bottling_unmapped.fl2d_20_21  "+
			""+
			"WHERE plan_id=? and  date_plan=? and  etin=? ";
...
public long getUploadSequence()
{
	...
	ResultSet rs=null;
	long val=0;
	String sql=    "select nextval('bwfl_license.cancel_permit_20_21')";
	try{
		conn=ConnectionToDataBase.getConnection();
		pstmt=conn.prepareStatement(sql);
		rs=pstmt.executeQuery()
...
public void dataCancel(Generate_BarCode_QRCode_For_CSD_Action action) {
	Connection conn1 = null;
	PreparedStatement pstmt1 = null;
	int status = 0;
	String update = "UPDATE fl2d.mst_stock_receive_20_21 SET cancel_req_flg='C' ,cancel_req_dt_time=?,cancel_req_reason='"+action.getRemark()+"' "
			+ "WHERE seq=?";
...
public int checkFl11GatepassBwfl(Generate_Bar_Code_QR_Code_For_CSD_DT dt)
{
	Connection conn=null;
	PreparedStatement pstmt=null;
	ResultSet rs=null;
	int i=0;
	String sql="SELECT  count(*) FROM bottling_unmapped.fl2d_20_21 where plan_id=? and  date_plan=? and  etin=? and fl11gatepass is null ";
	try{
		conn=ConnectionToDataBase.getConnection3();
		pstmt=conn.prepareStatement(sql);
...
public int checkFl36GatepassBwfl(Generate_Bar_Code_QR_Code_For_CSD_DT dt)
{
	Connection conn=null;
	PreparedStatement pstmt=null;
	ResultSet rs=null;
