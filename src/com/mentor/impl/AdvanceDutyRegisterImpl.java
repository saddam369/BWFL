...
import com.mentor.action.AdvanceDutyRegisterAction;
...
public class AdvanceDutyRegisterImpl {
	...
	public String getDetails(AdvanceDutyRegisterAction ac) {
		int id = 0;
		Connection con = null;
		...
		try {
			...
			//try {
				...
				//if(Integer.parseInt(ResourceUtil.getUserNameAllReq().trim())>0){
					queryList = " SELECT int_id,vch_license_type,vch_applicant_name, " +
							    " vch_firm_add,'BWFL' as logtype FROM bwfl.registration_of_bwfl_lic_holder_20_21 " +
							    " WHERE mobile_number='"+ ResourceUtil.getUserNameAllReq().trim()+ "' " +
							    " AND vch_approval='V'" +
							    " union " +
							    " select int_app_id as int_id,vch_license_type,vch_applicant_name, vch_core_address as vch_firm_add" +
							    " ,'FL2D' as logtype from licence.fl2_2b_2d_20_21 where vch_license_type='FL2D'" +
							    "  and vch_mobile_no='"+ ResourceUtil.getUserNameAllReq().trim()+ "'" ;
			...
			if (rs.next()) {
				...
				ac.setDist_id(rs.getInt("int_id"));
				ac.setAddress(rs.getString("vch_firm_add"));
				if(rs.getString("vch_license_type").equalsIgnoreCase("FL2D")){
					this.setLogin_type("FL2D");
				}
				else if(rs.getString("vch_license_type").equalsIgnoreCase("1")){
				     this.setLogin_type("BWFL2A");
				}else if(rs.getString("vch_license_type").equalsIgnoreCase("2")){
				     this.setLogin_type("BWFL2B");
				}else if(rs.getString("vch_license_type").equalsIgnoreCase("3")){
				     this.setLogin_type("BWFL2C");
				}else if(rs.getString("vch_license_type").equalsIgnoreCase("4")){
				     this.setLogin_type("BWFL2D");
				}
			} 
	...
	public ArrayList getSeasonDetails(AdvanceDutyRegisterAction ac) {
	...
	/*
	 * public static ArrayList getShowData_old(AdvanceDutyRegisterAction ac)
	 ...
	 * while (rs2.next()) {
	 * 
	 * AdvanceDutyRegisterDT dt= new
	 * AdvanceDutyRegisterDT();
	 * 
	 * dt.setSrNo(i);
	 * 
	...
	public ArrayList getShowData(AdvanceDutyRegisterAction ac) {
		ArrayList list = new ArrayList();
		...
		try {
			...
			if(this.getLogin_type().equalsIgnoreCase("FL2D")){	
				...
				if(ac.getType().equalsIgnoreCase("FL2D_Permit")){
					...
					filter2=" SELECT  b.cr_date, 0 as challan_amt, 'Import Fee on Permit ' || b.permit_nmbr as description, 'DT' as check_type,            "+  
							" SUM (b.import_fee) as disptch_duty FROM bwfl_license.import_permit_20_21 b WHERE b.bwfl_id='"+ac.getDist_id()+"' and login_type='FL2D'  "+                                                                
							...
							" 0 as disptch_duty FROM bwfl_license.chalan_deposit_bwfl_fl2d b WHERE b.unit_id='"+ ac.getDist_id()+ "' and unit_type='FL2D'                "+                                                 
						...
						filter2=" SELECT  b.cr_date, 0 as challan_amt, 'Special Fee on Permit ' || b.permit_nmbr as description, 'DT' as check_type,          "+    
								" SUM(b.special_fee) as disptch_duty FROM bwfl_license.import_permit_20_21 b WHERE b.bwfl_id='"+ac.getDist_id()+"' and login_type='FL2D' "+                                                                 
								" AND b.cr_date between '"+ Utility.convertUtilDateToSQLDate(ac.getFromdate())+ "'                                                                                     "+
								" AND '"+Utility.convertUtilDateToSQLDate(ac.getTodate())+"' and vch_approved='APPROVED'	                                                                       "+
								" GROUP BY  b.cr_date, description, check_type    " +
						        " UNION "+
						...
						filter2=
								...
								" a.vch_mill_name='"+ ac.getDist_id()+ "' and              "+
								...
								" AND b.int_fl2d_id='"+ac.getDist_id()+"'                                                                   "+
				...
				allData =" SELECT x.date_challan_date as dt , sum(x.challan_amt) as challan_amt , sum(x.disptch_duty) as disptch_duty,  "+
				        ...
				        " 'OP' as check_type, 0 as disptch_duty from fl2d.opening_fees_for_fl2d_bwfl_20_21 where unit_id='"+ ac.getDist_id()+ "' and unit_type='FL2D'         "+
                        " union "+
                        filter2 +
				        " UNION  "+
						...
						" WHERE   p.duty_type='"+ac.getType()+"' AND p.unit_id='"+ ac.getDist_id()+ "'                     "+
					...
					if(ac.getType().equalsIgnoreCase("BWFL_Import")){
						...
						filter2=" SELECT  b.cr_date, 0 as challan_amt, 'Import Fee on Permit ' || b.permit_nmbr as description, 'DT' as check_type,            "+  
								" SUM (b.import_fee) as disptch_duty FROM bwfl_license.import_permit_20_21 b WHERE b.bwfl_id='"+ac.getDist_id()+"' and login_type='BWFL'  "+                                                                
								" AND b.cr_date between '"+ Utility.convertUtilDateToSQLDate(ac.getFromdate())+ "'                                                                                     "+
								" AND '"+Utility.convertUtilDateToSQLDate(ac.getTodate())+"' and vch_approved='APPROVED'	                                                                         "+
								" GROUP BY  b.cr_date, description, check_type                                                                       "+
								" Union  " +
								" SELECT  b.cr_date, sum(amount) as challan_amt, 'Deposit Challan  - ' || b.chalan_no as description, 'CH' as check_type,  "+           
								" 0 as disptch_duty FROM bwfl_license.chalan_deposit_bwfl_fl2d b WHERE b.unit_id='"+ ac.getDist_id()+ "'   and unit_type like '%BWFL%'              "+                                                 
						...
						System.out.println("import ==="+filter2);
							...
							filter2=" SELECT b.date_crr_date, 0 as challan_amt, b.vch_description as description, 'DT' as check_type,              "+
									" SUM(b.int_value) as disptch_duty FROM distillery.duty_register_19_20 b WHERE b.vch_duty_type='DUTY_"+this.getLogin_type()+"'     "+
									" AND b.int_bwfl_id='"+ac.getDist_id()+"'                                                                   "+
									" AND b.date_crr_date between '"+ Utility.convertUtilDateToSQLDate(ac.getFromdate())+ "'                       "+
									" AND '"+ Utility.convertUtilDateToSQLDate(ac.getTodate())+ "' 	                                               "+
									" GROUP BY  b.date_crr_date, description, check_type  "+                                                                     
									" Union  " +
							...
							filter2=" SELECT  b.cr_date, 0 as challan_amt, 'Special Fee on Permit ' || b.permit_nmbr as description, 'DT' as check_type,          "+    
									" SUM(b.special_fee) as disptch_duty FROM bwfl_license.import_permit_20_21 b WHERE b.bwfl_id='"+ac.getDist_id()+"' and login_type='BWFL' "+                                                                 
									" AND b.cr_date between '"+ Utility.convertUtilDateToSQLDate(ac.getFromdate())+ "'                                                                                     "+
									" AND '"+Utility.convertUtilDateToSQLDate(ac.getTodate())+"' and vch_approved='APPROVED'	                                                                       "+
									" GROUP BY  b.cr_date, description, check_type    " +
							        " UNION "+
							...
							System.out.println("special ==="+filter2);
							            ...
							            filter2=
											...
											" WHERE  a.vch_duty_type='"+ac.getType()+"' AND  a.int_bwfl_id ='"+ ac.getDist_id()+ "'  "+
					...
					allData = 	" SELECT x.date_challan_date as dt , sum(x.challan_amt) as challan_amt , sum(x.disptch_duty) as disptch_duty,  "+
					        ...
					        " 'OP' as check_type, 0 as disptch_duty from fl2d.opening_fees_for_fl2d_bwfl_20_21 where unit_id='"+ ac.getDist_id()+ "' and unit_type='BWFL'         "+
					        " union" +
							 filter2 +
                            " union" +
						    " SELECT p.cr_date, 0 as challan_amt,                                                              "+
	...
	public void print(AdvanceDutyRegisterAction ac) {
...
public void getopening(AdvanceDutyRegisterAction ac) throws Exception {
