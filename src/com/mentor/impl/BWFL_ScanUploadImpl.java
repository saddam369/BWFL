...
import com.mentor.action.BWFL_ScanUploadAction;
...
public class BWFL_ScanUploadImpl {

	// -----------------------bwfl details---------------------

	public void getDetails(BWFL_ScanUploadAction act) {
		...
		String query = 	" SELECT int_id,int_distillery_id,vch_license_type,vch_license_number,vch_applicant_name, " +
						" vch_firm_add, vch_distillery_contact_number FROM bwfl.registration_of_bwfl_lic_holder_19_20 " +
	...
	public ArrayList getDetails_BWFL(BWFL_ScanUploadAction act) {
		...
		String selQr = " SELECT DISTINCT int_distillery_id ,permitno ,permitdt,gatepass_no,licence_no "
				+ " FROM bwfl_license.mst_bottling_plan_19_20 "
	...
	public ArrayList getFormDetails(BWFL_ScanUploadAction act, BWFL_ScanUploadDT dt) {
		...
		selQr = " SELECT DISTINCT a.unit_id,c.int_bwfl_id,a.int_distillery_id, a.int_brand_id, a.int_pack_id, a.int_quantity,       "+
				...
				" FROM bwfl_license.mst_bottling_plan_19_20 a ,bwfl.registration_of_bwfl_lic_holder_19_20 b,                        "+
				" distillery.brand_registration_19_20 c WHERE c.brand_id=a.int_brand_id                                             "+
	...
	public void saveExcelData(BWFL_ScanUploadAction action) {
		...
		try {
			String sql = " INSERT INTO bwfl_license.bwfl_dispatch_casecode(vch_gatepass_no, vch_casecode)VALUES (?, ?) ";

			conn = ConnectionToDataBase.getConnection();
			String query = "DELETE FROM bwfl_license.bwfl_dispatch_casecode where vch_gatepass_no ='"
	...
	public ArrayList getExcelData(BWFL_ScanUploadAction act) {
...
////System.out.println("getExcelData"+query);
	...
	public String getUnmappedGroupId(BWFL_ScanUploadAction act) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String groupId = "";
		...
		try {
			String query = 	" SELECT array_to_string(array_agg(group_id), ',') as group_id FROM bwfl_license.mst_bottling_plan_19_20  " +
	...
	public boolean getCascodeMatch(Date date1, String casecode,String etin ,BWFL_ScanUploadAction act)
	{
		Connection conn=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
	...
	public void deleteData(BWFL_ScanUploadAction act) {
	...
	public int excelCases(BWFL_ScanUploadAction act) {
		...
		try {
			...
			String query = 	" SELECT count(*) as dispatchd_box FROM bwfl_license.bwfl_dispatch_casecode " +
							"  WHERE vch_gatepass_no='"+ act.getGatepassForCsv().trim() + "'";
	...
	public int recieveCases(BWFL_ScanUploadAction act) {
		...
		try {
			...
			String query = " SELECT SUM(int_boxes) as dispatchd_box FROM bwfl_license.mst_bottling_plan_19_20 "
	...
	public boolean updateDispatch(BWFL_ScanUploadAction act) {
		int save = 0;
		int save1 = 0;
		boolean val = false;
		PreparedStatement ps = null;
		...
		try {
			...
			if (act.getGetVal().size() == save && save>0) {
				...
				String updtQr = " UPDATE bwfl_license.mst_bottling_plan_19_20 SET scan_upload_flag='F'  " +
				...
				delQr = " DELETE FROM bwfl_license.bwfl_dispatch_casecode " +
						" WHERE vch_gatepass_no ='"+ act.getGatepassForCsv().trim() + "' ";
	...
	/*public boolean updateDispatch_old(BWFL_ScanUploadAction act) {
		...
		try {
			...
			if (act.getGetVal().size() == save) {
			 ...
			 save = ps.executeUpdate(); 
				String query1 = " DELETE FROM bwfl_license.bwfl_dispatch_casecode WHERE vch_gatepass_no ='"
						+ act.getGatepassForCsv().trim() + "' ";
	...
	public ArrayList getRecieptsList(BWFL_ScanUploadAction act) {
		...
		selQr = " SELECT DISTINCT a.int_distillery_id, a.int_brand_id, a.int_pack_id, a.int_quantity, a.int_planned_bottles, "+
				...
				" FROM bwfl_license.mst_bottling_plan_19_20 a, distillery.brand_registration_19_20 b, distillery.packaging_details_19_20 c     "+
	...
	public boolean updateData(BWFL_ScanUploadAction act,BWFL_ScanUploadDT dt) {
		...
		try {
			...
			String gatepassNmbr = dt.getVch_license_type()+"_GATEPASS-"+dt.getSno();
				...
				if (dt.getBreakage() != "") {
					...
					/*String updtQr = " UPDATE bwfl_license.mst_bottling_plan_19_20 SET scan_upload_flag='S', "
	...
	public int getNewGatepass() {
		...
		String query = "SELECT MAX( CAST (gatepass_no AS INTEGER)) as gatepass_no FROM bwfl_license.mst_bottling_plan_19_20;";
	...
	public boolean printReport(BWFL_ScanUploadAction action) {
		...
		try {
			...
			reportQuery = "SELECT b.int_distillery_id,a.dispatch_date, a.gatepass_no,a.licence_no," +
					...
					" FROM bwfl_license.mst_bottling_plan_19_20 a, bwfl.registration_of_bwfl_lic_holder_19_20" +
					" b, distillery.brand_registration_19_20 c where a.gatepass_no='"+action.getGatepass()+"' and " +
...
////System.out.println("reportQuery="+reportQuery);
	...
	public void saveCSV(BWFL_ScanUploadAction action) throws IOException {
		Connection con = null;
		PreparedStatement stmt = null;
	...
	public boolean finalSubmit1(BWFL_ScanUploadAction act) {
	...
	//--------------------etin check--------------------------
		...
		public boolean  etin(String casecode, BWFL_ScanUploadAction act) {
			...
			try {
				...
				String query =	" SELECT distinct b.code_generate_through FROM bwfl_license.mst_bottling_plan_19_20 a, "+
		...
		public ArrayList getLicense()
		{
			...
			list.add(item);
			String sql=
					"SELECT vch_license_number,int_id,int_distillery_id FROM bwfl.registration_of_bwfl_lic_holder_19_20 "+
					"WHERE vch_distillery_contact_number='"+ ResourceUtil.getUserNameAllReq().trim() + "' "+
		...
		public void getLicAddress(BWFL_ScanUploadAction act) {
			Connection con = null;
			PreparedStatement ps = null;
			ResultSet rs = null;
			if(act.getSelect_lic_no()==null){
				act.setSelect_lic_no("0");
	 			...
	 			String query = 	"select vch_firm_add from bwfl.registration_of_bwfl_lic_holder_19_20 where int_id='"+act.getSelect_lic_no()+"'";
			try {
				con = ConnectionToDataBase.getConnection();
				ps = con.prepareStatement(query);
				rs = ps.executeQuery();
		...
		public boolean checkEtinNmbr(BWFL_ScanUploadAction act)
		{
			...
			try {
				...
				query = 	" SELECT DISTINCT etin, SUM(int_boxes) as box FROM bwfl_license.mst_bottling_plan_19_20   "+
							" WHERE gatepass_no='"+act.getGatepassForCsv().trim()+"' GROUP BY etin  " +
							" EXCEPT   "+
							" SELECT DISTINCT left(vch_casecode,12) as etin, count(*) as box FROM bwfl_license.bwfl_dispatch_casecode  "+
	...
	public int totalCasesOnPermit(BWFL_ScanUploadAction act) {
		...
		try {
			...
			String query = 	" SELECT SUM(int_boxes) as total_boxes FROM bwfl_license.mst_bottling_plan_19_20 " +
	...
	// =============count number of cases dispatched on the particular permit ===================
		...
		public int totalCasesDispatchedOnPermit(BWFL_ScanUploadAction act) {
