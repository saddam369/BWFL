...
import com.mentor.action.RollOverOfNonRenewalBrandAction;
...
public class RollOverOfNonRenewalBrandImpl {
	public String getDetails(RollOverOfNonRenewalBrandAction act) {
		int id = 0;
		Connection con = null;
	...
	public ArrayList displaylistImpl2(RollOverOfNonRenewalBrandAction act) {
		...
		try {
			...
			while (rs.next()) {
				...
				if (rs.getString("renwal_flg") != null
						&& rs.getString("renwal_flg").length() > 0
						&& rs.getString("renwal_flg").equalsIgnoreCase("T")) {
					dt.setFinl_flg(true);
					dt.setBtflag(false);
					// act.setDataflg(true);
	...
	// ---------------by arvind -----------------------------------
	public String viewdetail1(RollOverOfNonRenewalBrandAction act,
	...
	// ===========================code for csv============================
	/*
	 * public void saveCSV(RollOverOfNonRenewalBrandAction action) throws
	 * IOException {
	 * 
	 ...
	 * }
	 */
	public void saveCSV(RollOverOfNonRenewalBrandAction action)
			throws IOException {
		Connection con = null;
		...
		try {
			...
			String queryList = " delete FROM fl2d.rollover_stock_wholesale_non_renew_brand  where  etin='"
					+ action.getEtn()
					+ "' and unit_id="
			...
			String query = " INSERT INTO fl2d.rollover_stock_wholesale_non_renew_brand(etin,etin_new, casecode,unit_id,packg_id)VALUES (?, ?,?,?,?) ";
			String casecode = "";
			String gatepass = "";
			int status = 0, flag = 0;
			BufferedReader bReader = new BufferedReader(new FileReader(
	...
	public boolean etin(String casecode, RollOverOfNonRenewalBrandAction act) {
	...
	public ArrayList showcsvDetail(RollOverOfNonRenewalBrandAction act) {
		ArrayList list = new ArrayList();
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try {
			...
			String selQr1 = "select etin ,etin_new,casecode from  fl2d.rollover_stock_wholesale_non_renew_brand where etin='"
	...
	public void resetcsvcasecode(RollOverOfNonRenewalBrandAction action) {
		...
		try {
			...
			queryList = " delete FROM fl2d.rollover_stock_wholesale_non_renew_brand where unit_id="
					+ action.getFl2_fl2bId()
					+ " and etin='"
					+ action.getEtn()
					+ "'";
	...
	public String save(RollOverOfNonRenewalBrandAction act,
			rollover_Stock_20_21_fl2_dt dt) {
		Connection con = null;
		...
		try {
			 ...
			 */{
				...
				query = "INSERT INTO fl2d.rollover_fl_stock_non_renew_brand(etin,etin_new, brand_id, package_id, "
						+ " rollover_bottles, rollover_boxes, finalize_dt,unit_id,unit_type,old_unit_id)"
						+ " VALUES ('"
						+ act.getEtn()
						+ "', '"
						...
						+ "','"
						+ act.getFl2_fl2bId() + "','"+dt.getType_old()+"','"+dt.getUnit_id_old()+"'"+")";
	...
	public String getDetails1(RollOverOfNonRenewalBrandAction ac,
			rollover_Stock_20_21_fl2_dt dt) {
	...
	public String viewdetail11(RollOverOfNonRenewalBrandAction act,
			rollover_Stock_20_21_fl2_dt dt) {
	...
	public String viewdetail12(RollOverOfNonRenewalBrandAction act,
			rollover_Stock_20_21_fl2_dt dt) {
	...
	public boolean validatechallan(RollOverOfNonRenewalBrandAction action,
			rollover_Stock_20_21_fl2_dt dt) {
		boolean flag = false;
		Connection conn = null;
		ResultSet rs = null;
	...
	public boolean updatechallandetail(RollOverOfNonRenewalBrandAction action,
			rollover_Stock_20_21_fl2_dt dt) {
		ArrayList list = new ArrayList();
		boolean flag = false;
		Connection conn = null;
	...
	public int displaylist1implcount(RollOverOfNonRenewalBrandAction act) {
		...
		try {
			...
			query = " select distinct count(*) from  fl2d.rollover_stock_wholesale_non_renew_brand where etin='"
					+ act.getEtn() + "' and unit_id=" + act.getFl2_fl2bId();
	...
	public String finalize(RollOverOfNonRenewalBrandAction act,
			rollover_Stock_20_21_fl2_dt dt) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			String query = " update fl2d.rollover_fl_stock_non_renew_brand set deo_flg ='T' where "
					+ " brand_id='"
					+ dt.getBrand_id()
					+ "' and package_id='"
					+ dt.getPackage_id()
