...
import com.mentor.action.GatepassToDistrict_FL2_FL2B_activeAction;
import com.mentor.datatable.GatepassToDistrict_FL2_FL2B_activeDt;
import com.mentor.datatable.GatepassToDistrict_FL2_FL2B_active_check_datatable;
...
public class GatepassToDistrict_FL2_FL2B_activeImpl {

	// =====================get fl2_fl2b
	// details=================================

	public String getDetails(GatepassToDistrict_FL2_FL2B_activeAction act) {
	...
	public ArrayList getbrcLicenseNo(GatepassToDistrict_FL2_FL2B_activeAction act) {
	...
	public ArrayList getlicenseNmbr(GatepassToDistrict_FL2_FL2B_activeAction act) {
	...
	public ArrayList getDistrictList(GatepassToDistrict_FL2_FL2B_activeAction act) {
		...
		try {
			...
			String query = " SELECT distinct coalesce(a.districtid,0) as districtid , a.description FROM public.district a ,licence.fl2_2b_2d_20_21 b"
					+ " where  a.districtid=b.core_district_id  and b.loginid='"
					+ ResourceUtil.getUserNameAllReq().trim() + "'";
	...
	public String getlicenseeDetail(GatepassToDistrict_FL2_FL2B_activeAction act,
			String val) {
	...
	public String getretailLicenseeDetail(
			GatepassToDistrict_FL2_FL2B_activeAction act, String val) {
	...
	public ArrayList displaylistImpl(GatepassToDistrict_FL2_FL2B_activeAction act) {
		...
		try {
			if (act.getVch_from().equalsIgnoreCase("FL2")) {
				selQr = "  SELECT a.package_name, b.brand_name,b.unit_nm, "
						...
						+ "	    c.pckg_id,  c.size  as box_size ,b.license_category                                                    "
						+ "	    FROM distillery.packaging_details_20_21 a   ,                                         "
						+ "	    distillery.brand_registration_20_21 b,fl2d.fl2_2b_stock_20_21   c                                                         "
						+ "	    WHERE a.brand_id_fk=b.brand_id and (a.edp+a.mrp+a.permit)>0 AND a.brand_id_fk=c.brand_id                                                   "
						...
						+ " b.brand_name,c.size , b.distillery_id, b.brewery_id, b.int_bwfl_id,b.int_fl2d_id,b.unit_nm,b.license_category       "
			 ...
			 * +
			 * " c.pckg_id, (sum(c.total_recv_bottels) -coalesce((select sum(coalesce(x.dispatch_bottle,0)) from fl2d.fl2_stock_trxn_fl2_fl2b_20_21 x"
			 ...
			 * " FROM distillery.packaging_details_20_21 a                                                                               "
			 * +
			 * " left outer join distillery.box_size_details d on d.box_id=a.box_id  ,   distillery.brand_registration_20_21 b,          "
			 * +
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				GatepassToDistrict_FL2_FL2B_activeDt dt = new GatepassToDistrict_FL2_FL2B_activeDt();
                ...
                total_bottle=total_bottle+rs.getInt("avlbottle");
                dt.setLicense_category(rs.getString("license_category"));
				list.add(dt);
				i++;
			}
			
			act.setTotal_avl_bottle(total_bottle);
	...
	public int maxId() {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String query = " SELECT max(seq) as id FROM fl2d.gatepass_to_districtwholesale_fl2_fl2b_20_21 ";
		int maxid = 0;
		try {
			con = ConnectionToDataBase.getConnection();
	...
	public String saveMethodImpl(GatepassToDistrict_FL2_FL2B_activeAction act) {
		...
		String gatepass = String.valueOf(act.getFl2_fl2bId()) + "-2020-21-"+ act.getFl2LicenseType().trim() + "-" + seq;
		...
		try {
			...
			String insQr = " INSERT INTO fl2d.gatepass_to_districtwholesale_fl2_fl2b_20_21( "
			...
			else if (act.vch_to.equalsIgnoreCase("RT") || act.vch_to.equalsIgnoreCase("PR") ) {
			...
			if (act.vch_to.equalsIgnoreCase("RT") || act.vch_to.equalsIgnoreCase("PR")) {
			...
			if (tok1 > 0) {
				...
				for (int i = 0; i < act.getDisplaylist().size(); i++) {
					...
					GatepassToDistrict_FL2_FL2B_activeDt dt = (GatepassToDistrict_FL2_FL2B_activeDt) act
							.getDisplaylist().get(i);
					if (dt.getDispatchbottls() > 0 && dt.getDispatchbox() > 0) {
						tok1 = 0;

						String insQr1 = " INSERT INTO fl2d.fl2_stock_trxn_fl2_fl2b_20_21( "
						...
						if (tok1 > 0) {
							...
							String updtQr = " UPDATE fl2d.fl2_2b_stock_20_21 "
	...
	public ArrayList getTable2List1(GatepassToDistrict_FL2_FL2B_activeAction act,
			Date ev) {
		ArrayList list = null;
		Connection con = null;
		PreparedStatement ps = null;
	...
	public ArrayList getTable2List(GatepassToDistrict_FL2_FL2B_activeAction act) {
		ArrayList list = null;
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public String getEmailDetails(GatepassToDistrict_FL2_FL2B_activeAction act) {
		...
		try {
			...
			String queryList = " SELECT a.officer_email "
					+ " FROM public.district a, fl2d.gatepass_to_districtwholesale_fl2_fl2b_20_21 b "
	...
	/*
	 * public boolean printReport(GatepassToDistrict_FL2_FL2B_activeAction act,
	 * GatepassToDistrict_FL2_FL2B_activeDt dt) {
	 * 
	 ...
	 * +
	 * " FROM fl2d.gatepass_to_districtwholesale_fl2_fl2b_20_21 a, licence.fl2_2b_2d_20_21 b, "
	 * +
	 * " fl2d.fl2_stock_trxn_fl2_fl2b_20_21 c, distillery.brand_registration_20_21 d, "
	 * + " distillery.packaging_details_20_21 e " +
	 ...
	 * // -------------------------print draft--------------------------
	 * 
	 * public boolean printDraftReport(GatepassToDistrict_FL2_FL2B_activeAction act,
	 * GatepassToDistrict_FL2_FL2B_activeDt dt) {
	 * 
	 * String mypath = Constants.JBOSS_SERVER_PATH + Constants.JBOSS_LINX_PATH;
	 * String relativePath = mypath + File.separator + "ExciseUp" +
	 ...
	 * +
	 * " fl2d.fl2_stock_trxn_fl2_fl2b_20_21 c, distillery.brand_registration_20_21 d, "
	 * + " distillery.packaging_details_20_21 e, distillery.box_size_details f "
	 * +
	 * " WHERE a.int_fl2_fl2b_id=b.int_app_id AND a.int_fl2_fl2b_id=c.int_fl2_fl2b_id "
	 * + " AND a.vch_gatepass_no=c.vch_gatepass_no and a.dt_date=c.dt " +
	 * " AND c.int_brand_id=d.brand_id AND d.brand_id=e.brand_id_fk " +
	 ...
	 */
	/*
	 * public void saveExcelData(GatepassToDistrict_FL2_FL2B_activeAction action) {
	...
	public void saveExcelDatafl(GatepassToDistrict_FL2_FL2B_activeAction action) {
		String gatepass = "";
		int status = 0, flag = 1, excelcase = 0;
		int k[] = null;
		Connection conn = null;
	...
	public void deleteData(GatepassToDistrict_FL2_FL2B_activeAction action) {
		Connection con = null;
		PreparedStatement stmt = null;
	...
	public int recieveCases(GatepassToDistrict_FL2_FL2B_activeAction act) {
		int id = 0;
		Connection con = null;
		PreparedStatement pstmt = null, ps2 = null;
		ResultSet rs = null, rs2 = null;
		...
		try {
			...
			String query = " SELECT SUM(dispatch_box) as dispatchd_box FROM fl2d.fl2_stock_trxn_fl2_fl2b_20_21 "
	...
	public int excelCases(GatepassToDistrict_FL2_FL2B_activeAction act) {
		int id = 0;
		Connection con = null;
		PreparedStatement pstmt = null, ps2 = null;
		ResultSet rs = null, rs2 = null;
	...
	public boolean labcheck(String casecode) {
		...
		try {
			...
			String query = " SELECT distinct a.license_category FROM distillery.brand_registration_20_21 a, distillery.packaging_details_20_21 b "
	...
	public boolean etin(String casecode, GatepassToDistrict_FL2_FL2B_activeAction act) {
		...
		try {
			...
			String query = " SELECT distinct b.code_generate_through FROM fl2d.fl2_stock_trxn_fl2_fl2b_20_21 a, distillery.packaging_details_20_21 b "
	...
	// ----------------------------update
	public boolean updateFL3(GatepassToDistrict_FL2_FL2B_activeAction act) {
		int save = 0;
		boolean val = false;
		PreparedStatement ps = null;
		Connection con = null;
		String updtQr = " UPDATE fl2d.gatepass_to_districtwholesale_fl2_fl2b_20_21 SET  vch_finalize='F', finalize_dt_time=? "
	...
	// Fl3-------------------------------------------
	/*
	 * public boolean updateFL3(GatepassToDistrict_FL2_FL2B_activeAction act) { int
	 ...
	 * ConnectionToDataBase.getConnection20_21(); con.setAutoCommit(false);
	 * con1.setAutoCommit(false); ps = con1.prepareStatement(query); /* for (int
	 * i = 0; i < act.getGetVal().size(); i++) {
	 * 
	 * GatepassToDistrict_FL2_FL2B_activeDt dt = (GatepassToDistrict_FL2_FL2B_activeDt)
	 * act.getGetVal().get(i);
	 * 
	 ...
	 * " UPDATE fl2d.gatepass_to_districtwholesale_fl2_fl2b_20_21 SET  vch_finalize='F', finalize_dt_time=? "
	 * + " WHERE vch_gatepass_no='"+ act.getScanGatePassNo() + "'";
	 * 
	...
	public boolean etin_check(String g) {
		...
		String q2 = "select b.f*count(b.code_generate_through) as count, b.code_generate_through as etin from"
				...
				+ " (select dispatch_box as f,int_pckg_id as p  from fl2d.fl2_stock_trxn_fl2_fl2b_20_21"
				+ " where vch_gatepass_no = '"
				+ g
		...
		// System.out.println("second datatable==============="+query);
		try {
			c1 = ConnectionToDataBase.getConnection20_21();
	 ...
	 * i++) { GatepassToDistrict_FL2_FL2B_activeDt dt =
	 * (GatepassToDistrict_FL2_FL2B_activeDt) act .getGetVal().get(i);
	 * 
	 ...
	 * "UPDATE fl2d.gatepass_to_districtwholesale_fl2_fl2b_20_21 SET  vch_finalize='F' WHERE vch_gatepass_no='"
	 * + act.getScanGatePassNo() + "'";
	 * 
	...
	public String cancelGatepassImpl(GatepassToDistrict_FL2_FL2B_activeAction act) {
		...
		try {
			...
			if (tok1 > 0) {
				...
				sql2 = " DELETE FROM fl2d.gatepass_to_districtwholesale_fl2_fl2b_20_21 "
			...
			if (tok1 > 0) {
				...
				sql3 = " DELETE FROM fl2d.fl2_stock_trxn_fl2_fl2b_20_21 "
	...
	public void saveCSV(GatepassToDistrict_FL2_FL2B_activeAction action)
			throws IOException {
		Connection con = null;
		PreparedStatement stmt = null;
		action.getVal.clear();
...
System.out.println("action.getFl2_fl2bId() "+action.getFl2_fl2bId());
	...
	public void saveCSVfl(GatepassToDistrict_FL2_FL2B_activeAction action)
			throws IOException {
		Connection con = null;
		PreparedStatement stmt = null;
	...
	public boolean printReport_CaseCode(GatepassToDistrict_FL2_FL2B_activeAction act,
			GatepassToDistrict_FL2_FL2B_activeDt dt) {
		...
		try {
			// con = ConnectionToDataBase.getConnection();
			con3 = ConnectionToDataBase.getConnection20_21();
	...
	public int getDetail(GatepassToDistrict_FL2_FL2B_activeAction act, String gatepass) {
		...
		try {
			...
			while (rs.next()) {
				...
				updtQr11 = " UPDATE fl2d.fl2_2b_stock_20_21 "
	...
	public boolean printReport(GatepassToDistrict_FL2_FL2B_activeAction act,
		...
		try {
			con = ConnectionToDataBase.getConnection();
			reportQuery = "select * from fl2d.fl2_2b_gatepass_20_21('"
			...
			if (rs.next()) {
				...
				if (act.getVchToPrint().equalsIgnoreCase("RT") || act.getVchToPrint().equalsIgnoreCase("PR")) {
	...
	public boolean printDraftReport(GatepassToDistrict_FL2_FL2B_activeAction act,
		...
		try {
			...
			if (rs.next()) {
				...
				if (act.getVch_TO_Print().equalsIgnoreCase("RT") || act.getVch_TO_Print().equalsIgnoreCase("PR")) {
	...
	public void getShopDetails(GatepassToDistrict_FL2_FL2B_activeAction ac) {
		...
		try {
			con = ConnectionToDataBase.getConnection();
        if(ac.getVch_to().equalsIgnoreCase("RT")){
			if (ac.vch_from.equalsIgnoreCase("CL2")) {
				 sql = " SELECT district_id , vch_name_of_shop,vch_shop_type, "
				...
				sql = " SELECT district_id , vch_name_of_shop,vch_shop_type, "
						...
						+ ac.getDistrictLic()
						+ "' "
						+ "  and  vch_shop_type in ('Model Shop','Foreign Liquor','Beer')   ";
        ...
        }else if (ac.getVch_to().equalsIgnoreCase("PR")){
        	
        	sql =  " select shop_dist_id as district_id, shop_nm as vch_name_of_shop ,shop_ad as vch_location_of_shop ,shop_type as vch_shop_type" +
        		   " ,case when licence_type='Individual' then licencee_name                           "+
                   " when licence_type='Firm' then  licencee_name_1 || ', ' ||  licencee_name_2       "+
                   " when licence_type='Company' then  company_name end as vch_name_of_licensee from retail.premium_retail_registration " +
                   " where shop_dist_id ='"+ac.getDistrictLic()+"' and shop_id='"+ac.getShopno().trim()+"'";
        }
			int i = 0;
			 
			System.out.println("shop type=="+sql);
			...
			while (rs.next()) {
				...
				ac.setShopTyp(rs.getString("vch_shop_type"));
				ac.setFlagshop(true);
	...
	public boolean check_economy(int id) {
		...
		try {
			...
			String sql = "select * from distillery.brand_registration_20_21 where liquor_category='1' and brand_id='"
					+ id + "' ";
			pstmt = con.prepareStatement(sql);
	...
	public boolean check_wine(int id) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
boolean flg=false;
		try {
			...
			String sql = "select * from distillery.brand_registration_20_21 where liquor_type in ('WINE','IMPORTED WINE') and brand_id='"
					+ id + "' ";
			pstmt = con.prepareStatement(sql); 
			rs = pstmt.executeQuery();
	...
	public String checklic(GatepassToDistrict_FL2_FL2B_activeAction ac) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String f = "N";
	...
	public String checkshop(GatepassToDistrict_FL2_FL2B_activeAction ac) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String f = "";
	...
	public ArrayList getGatepassDetail(GatepassToDistrict_FL2_FL2B_activeAction action)
	{
		...
		String sql=
				...
				" select code_generate_through,brand_name,dispatch_bottle,dispatch_box,vch_gatepass_no  "+
				" from fl2d.fl2_stock_trxn_fl2_fl2b_20_21 a ,                                           "+
				" distillery.packaging_details_20_21 b ,                                                "+
				" distillery.brand_registration_20_21 c                                                 "+
				" where b.brand_id_fk=c.brand_id and a.int_brand_id=b.brand_id_fk                       "+
				" and a.int_pckg_id=b.package_id and vch_gatepass_no='"+action.getScanGatePassNo()+"'";
		...
		try{
			...
			while(rs.next())
			{
			GatepassToDistrict_FL2_FL2B_active_check_datatable dt=new GatepassToDistrict_FL2_FL2B_active_check_datatable();
			dt.setEtin(rs.getString("code_generate_through"));
			dt.setFrequency(rs.getInt("dispatch_box"));
	...
	public boolean  checkUploadedData(GatepassToDistrict_FL2_FL2B_activeAction action)
	{
		ArrayList list=new ArrayList();
		...
		try{
			...
			while ((line = bReader.readLine()) != null) {
				...
				while (st.hasMoreTokens()) {
					...
					if (sd != null) {
						...
						{
						list.add(sd.trim().substring(0, 12));	
						}
						}
				}
			...
			ArrayList l= getGatepassDetail(action);
			System.out.println(l.size()+" sizeeeee"+" list "+list);
			for(int k=0;k<l.size();k++)
			{
				GatepassToDistrict_FL2_FL2B_active_check_datatable dt=(GatepassToDistrict_FL2_FL2B_active_check_datatable)l.get(k);
				String etin=dt.getEtin();
				int freq=dt.getFrequency();
				String brand_name=dt.getBrand_name();
				
				
				int freque = Collections.frequency(list, etin);
				
				if(freq==freque)
				{
					
				}else{
					FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("Gatepass Generated "+freq +"Cases For "+brand_name+" For This Brand  Uploaded in Csv "+freque+" Please Check Csv And Uploaded Again " , "Gatepass Generated "+freq +"Cases For "+brand_name+" Uploaded in Csv "+freque+" Please Check Csv And Uploaded Again" ));
				check++;
				}
			}
	...
	public double getAdvanceSpclDuty(GatepassToDistrict_FL2_FL2B_activeAction act) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		double duty=0.0;
		...
		try {
			con = ConnectionToDataBase.getConnection();
            if(act.getFl2LicenseType().equalsIgnoreCase("FL2")){
			 sql = " select sum(x.vch_total_amount)+sum(x.opening)- sum(x.dispatch_amt) as advance from                                                                               "+
                   " (select coalesce(sum(vch_total_amount::numeric),0) as vch_total_amount,0 as opening, 0 as  dispatch_amt                                                         "+
                   " from licence.mst_challan_master a,licence.challan_head_details b                                                                                              "+
                   " where a.vch_challan_id=b.vch_challan_id and b.vch_rajaswa_head in ('3900105020000') and b.g6_head in (19) and a.dat_created_date>'2020-03-31'                                                    "+
                   " and a.vch_mill_name='"+act.getVch_to_lic_no()+"' and a.vch_mill_type='HotelBarRest' and vch_register_flag='R' and a.vch_trn_status='SUCCESS'                                         "+
                   " union                                                                                                                                                         "+
                   " select 0 as vch_total_amount ,coalesce(sum(fl_amt),0) as opening,0 as dispatch_amt  from hotel_bar_rest.opening_amt_for_hbr_20_21                               "+
                   " where hbr_id='"+act.getVch_to_lic_no()+"'                                                                                                                                              "+
                   " union                                                                                                                                                         "+
                   " select 0 as vch_total_amount,0 as opening,case when br.liquor_category='1' then "+
                   " 0 else coalesce(sum(b.dispatch_bottle),0)*10 end as dispatch_amt from fl2d.gatepass_to_districtwholesale_fl2_fl2b_20_21 a,  "+
                   " fl2d.fl2_stock_trxn_fl2_fl2b_20_21 b, distillery.brand_registration_20_21 br, distillery.packaging_details_20_21 pk                                           "+
                   " where a.int_fl2_fl2b_id=b.int_fl2_fl2b_id and a.vch_gatepass_no=b.vch_gatepass_no and br.brand_id=pk.brand_id_fk                                              "+
                   " and br.brand_id=b.int_brand_id and pk.package_id=b.int_pckg_id  and                                                                                           "+
                   " vch_to_lic_no='"+act.getVch_to_lic_no()+"' and vch_to in ('BRC') "+
                   " and br.license_category in ('IMFL','IMPORTED FL','IMPORTED WINE','WINE') group by br.liquor_category " +                                                                                                                                       
                   " union       "+                                                                  
                   " select 0 as vch_total_amount,0 as opening,case when br.liquor_category='1' then "+
                   " 0 else coalesce(sum(b.dispatch_bottle),0)*10 end as dispatch_amt from fl2d.gatepass_to_districtwholesale_fl2d_20_21 a,  "+
                   " fl2d.fl2d_stock_trxn_20_21 b, distillery.brand_registration_20_21 br, distillery.packaging_details_20_21 pk                                           "+
                   " where a.int_fl2d_id=b.int_fl2d_id and a.vch_gatepass_no=b.vch_gatepass_no and br.brand_id=pk.brand_id_fk                                              "+
                   " and br.brand_id=b.int_brand_id and pk.package_id=b.int_pckg_id  and                                                                                           "+
                   " vch_to_lic_no='"+act.getVch_to_lic_no()+"' and vch_to in ('HBR') "+
                   " and br.license_category in ('IMFL','IMPORTED FL','IMPORTED WINE','WINE') group by br.liquor_category " +
                   " )x";
            }
            else  if(act.getFl2LicenseType().equalsIgnoreCase("FL2B")){
            	...
            	sql="select sum(x.vch_total_amount)+sum(x.opening)- sum(x.dispatch_amt) as advance from                                                                             "+
                    " (select coalesce(sum(vch_total_amount::numeric),0) as vch_total_amount,0 as opening, 0 as  dispatch_amt                                                       "+
                    " from licence.mst_challan_master a,licence.challan_head_details b                                                                                            "+
                    " where a.vch_challan_id=b.vch_challan_id and b.vch_rajaswa_head in ('3900103020000') and b.g6_head in (14)  and a.dat_created_date>'2020-03-31'                                                  "+
                    " and a.vch_mill_name='"+act.getVch_to_lic_no()+"' and a.vch_mill_type='HotelBarRest' and vch_register_flag='R' and a.vch_trn_status='SUCCESS'                                       "+
                    " union                                                                                                                                                       "+
                    " select 0 as vch_total_amount ,coalesce(sum(beer_amt),0) as opening,0 as dispatch_amt  from hotel_bar_rest.opening_amt_for_hbr_20_21                             "+
                    " where hbr_id='"+act.getVch_to_lic_no()+"'                                                                                                                                            "+
                    " union                                                                                                                                                       "+
                    " select 0 as vch_total_amount,0 as opening,case when br.liquor_category='1' then"+
                    " 0 else coalesce(sum(b.dispatch_bottle),0)*5 end as dispatch_amt from fl2d.gatepass_to_districtwholesale_fl2_fl2b_20_21 a,"+
                    " fl2d.fl2_stock_trxn_fl2_fl2b_20_21 b, distillery.brand_registration_20_21 br, distillery.packaging_details_20_21 pk                                         "+
                    " where a.int_fl2_fl2b_id=b.int_fl2_fl2b_id and a.vch_gatepass_no=b.vch_gatepass_no and br.brand_id=pk.brand_id_fk                                            "+
                    " and br.brand_id=b.int_brand_id and pk.package_id=b.int_pckg_id  and                                                                                         "+
                    " vch_to_lic_no='"+act.getVch_to_lic_no()+"' and vch_to in ('BRC')                                                                                      "+
                    " and br.license_category in ('BEER','IMPORTED BEER','LAB') group by br.liquor_category " +
                    " union                                                                                                                                                       "+
                    " select 0 as vch_total_amount,0 as opening,case when br.liquor_category='1' then"+
                    " 0 else coalesce(sum(b.dispatch_bottle),0)*5 end as dispatch_amt from fl2d.gatepass_to_districtwholesale_fl2d_20_21 a,"+
                    " fl2d.fl2d_stock_trxn_20_21 b, distillery.brand_registration_20_21 br, distillery.packaging_details_20_21 pk                                         "+
                    " where a.int_fl2d_id=b.int_fl2d_id and a.vch_gatepass_no=b.vch_gatepass_no and br.brand_id=pk.brand_id_fk                                            "+
                    " and br.brand_id=b.int_brand_id and pk.package_id=b.int_pckg_id  and                                                                                         "+
                    " vch_to_lic_no='"+act.getVch_to_lic_no()+"' and vch_to in ('HBR')                                                                                      "+
            ...
            System.out.println("advance duty quryy=="+sql);
			pstmt = con.prepareStatement(sql);

			rs = pstmt.executeQuery();

			if (rs.next()) {
				duty=rs.getDouble("advance");
			}
	...
	public boolean checkPRBrand(GatepassToDistrict_FL2_FL2B_activeAction act){
			...
			for (int i = 0; i < act.getDisplaylist().size(); i++) {
				...
				if (dt.getDispatchbottls() > 0 && dt.getDispatchbox() > 0) {
					String sql=
							...
							" select * from distillery.packaging_details_20_21 where brand_id_fk='"+dt.getInt_brand_id()+"'" +
							" and package_id='"+dt.getInt_pckg_id()+"' and premium_retail is true";
				
					System.out.println("PR Brand===="+sql);
			...
			if(rs.next())
			{
		       flag=true;
				...
				FacesContext.getCurrentInstance().addMessage(
						null,
						new FacesMessage(FacesMessage.SEVERITY_ERROR,
								"Brand "+dt.getBrandUnitName_dt()+" is not allowed for Premium Retail !!",
								"Brand "+dt.getBrandUnitName_dt()+" is not allowed for Premium Retail !!"));
				 flag=false;
				 break;
			}
...
public boolean checkmgq(GatepassToDistrict_FL2_FL2B_activeAction act){
		...
		try{
			if((act.getDistrictLic()==17 || act.getDistrictLic()==16 ||act.getDistrictLic()==13 ||act.getDistrictLic()==18 ||act.getDistrictLic()==3||act.getDistrictLic()==2||act.getDistrictLic()==10) )
			{
				if(act.getShopno().equalsIgnoreCase("11775") ||act.getShopno().equalsIgnoreCase("11840"))
				{
					flag=true;
				}	
				else {
					String sql=
							...
							" select   case when bl>0 and coalesce(vch_yearly_mgq,0)!=0 then bl*100/(coalesce(vch_yearly_mgq,0)/12) else '0' end as percent"
							+ " from"
							+ " (select EXTRACT(MONTH FROM xx.dispatch_date),shopid,sum(coalesce(bl_1,0))+sum(coalesce(bl_2,0))+sum(coalesce(bl_3,0)) bl "
							+ "from (select m.dispatch_date,m.shopId,sum(bl) as bl,m.strength,  case when  m.strength=25 then round(CAST(float8(sum(bl))as numeric),2)*25/36 end as bl_1," + 
							" case when  m.strength=36 then sum(bl) end as bl_2 ,  case when  m.strength=42.80 then round(CAST(float8(sum(bl))as numeric),2)*42.80/36 end as bl_3  "
							+ "from                                   "
							+ " (select x.dispatch_date,x.shopId,(((x.bottal)*f.qnt_ml_detail)/1000) as bl,d.strength from  (select b.int_pckg_id as package_id,b.int_brand_id, a.dt_date as dispatch_date,a.shop_id as shopId,sum(b.dispatch_bottle) as bottal" + 
							" from fl2d.gatepass_to_districtwholesale_fl2_fl2b_20_21 a,fl2d.fl2_stock_trxn_fl2_fl2b_20_21 b" + 
							" where  a.vch_from='CL2' and b.vch_gatepass_no=a.vch_gatepass_no and a.shop_id!='0' and a.shop_id is not null  group by a.shop_id, a.dt_date,b.int_pckg_id,b.int_brand_id                     union" + 
							...
							" where  a.vch_from='CL2' and b.vch_gatepass_no=a.vch_gatepass_no and  dt_date>'2020-04-01' and a.shop_id!='0' and a.shop_id is not null" + 
							...
							" where x.int_brand_id=d.brand_id and e.box_id=f.box_id and x.package_id=e.package_id)m" + 
							" group by m.dispatch_date,m.shopId ,m.strength)xx where xx.shopid='"+act.getShopno()+"' and EXTRACT(MONTH FROM xx.dispatch_date)=EXTRACT(MONTH FROM CURRENT_DATE) group by shopid,EXTRACT(MONTH FROM xx.dispatch_date))y,retail.retail_shop r where y.shopid=r.sequence_no;";
			...
			if(rs.next())
			{
				if(rs.getDouble(1)>105) {
					flag=false;
					FacesContext.getCurrentInstance().addMessage(
							null,
							new FacesMessage(FacesMessage.SEVERITY_ERROR,
									"Monthly MGQ Increase should not be more than 5% and actual is "+rs.getDouble(1)+"% !!",
									"Monthly MGQ Increase should not be more than 5% and actual is "+rs.getDouble(1)+"% !!"));
				}else {
					 flag=true;
				}
