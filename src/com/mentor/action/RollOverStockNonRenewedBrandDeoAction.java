...
import com.mentor.impl.RollOverStockNonRenewedBrandDeoImpl;

public class RollOverStockNonRenewedBrandDeoAction {
	RollOverStockNonRenewedBrandDeoImpl impl = new RollOverStockNonRenewedBrandDeoImpl();
...
private String radio_type="WS";

Rollover_Stock_20_21_fl2_DEODT deoDt;


public Rollover_Stock_20_21_fl2_DEODT getDeoDt() {
	return deoDt;
}
public void setDeoDt(Rollover_Stock_20_21_fl2_DEODT deoDt) {
	this.deoDt = deoDt;
}
...
public boolean isValidateInput() {
	  ...
	  this.validateInput=true;
			...
			for (int i = 0; i < this.getChallantable().size(); i++) {
				Rollover_Stock_20_21_fl2_DEODT dt1 = (Rollover_Stock_20_21_fl2_DEODT) this
...
public String finalizeData()
{
	...
	try{
		...
		if(!new RollOverStockNonRenewedBrandDeoImpl().validatechallan(this,dt)){
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("Challan  Not Found ! ", "Challan  Not Found!"));	
		}else
		{
			
		boolean flag=new RollOverStockNonRenewedBrandDeoImpl().updatechallandetail(this,dt);
		if(flag)
		{
...
UIDataTable uiTable = (UIDataTable) e.getComponent().getParent()
		.getParent();
Rollover_Stock_20_21_fl2_DEODT dt = (Rollover_Stock_20_21_fl2_DEODT) this.getChallantable().get(uiTable.getRowIndex());
		...
		int count=0;
			 ...
			 if(this.getDisplaylist().size()>0)
				...
				for(int i=0;i< this.getDisplaylist().size(); i++){
					...
					Rollover_Stock_20_21_fl2_DEODT	 dt = (Rollover_Stock_20_21_fl2_DEODT) this.getDisplaylist().get(i);
...
{
	try{
		if(deoDt.getUnit_type().trim().equalsIgnoreCase("BWFL"))
		{
		new RollOverStockNonRenewedBrandDeoImpl().dataFinalizeBwfl(this, deoDt);
		this.displaylist=impl.displaylist(this);
		}else if(deoDt.getUnit_type().trim().equalsIgnoreCase("B"))
		{
		new RollOverStockNonRenewedBrandDeoImpl().dataFinalizeBrewary(this, deoDt);
		this.displaylist=impl.displaylist(this);
		}
		else if(deoDt.getUnit_type().trim().equalsIgnoreCase("D"))
		{
		new RollOverStockNonRenewedBrandDeoImpl().dataFinalizeDistillery(this, deoDt);
		this.displaylist=impl.displaylist(this);
		}
		else if(deoDt.getUnit_type().trim().equalsIgnoreCase("OtherUnit"))
		{
		new RollOverStockNonRenewedBrandDeoImpl().dataFinalizefl2d(this, deoDt);
