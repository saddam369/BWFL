...
public class Generate_BarCode_QRCode_For_CSD_Action {
...
private Generate_Bar_Code_QR_Code_For_CSD_DT datatable;
	public Generate_Bar_Code_QR_Code_For_CSD_DT getDatatable() {
	return datatable;
}

public void setDatatable(Generate_Bar_Code_QR_Code_For_CSD_DT datatable) {
	...
	public ArrayList getUnitlist() {
		...
		unitlist = new Generate_BarCode_QRCode_For_CSD_Impl().getUnit();
		return unitlist;
	}
	...
	public void deleteRowMethodPackaging(ActionEvent e) {
		UIDataTable uiTable = (UIDataTable) e.getComponent().getParent()
				.getParent();
		Generate_Bar_Code_QR_Code_For_CSD_DT dt = (Generate_Bar_Code_QR_Code_For_CSD_DT) this.brandPackagingDataList
		...
		if (validateInput) {
			if (this.brandPackagingDataList.size() > 0) {
				for (int i = 0; i < brandPackagingDataList.size(); i++) {
					Generate_Bar_Code_QR_Code_For_CSD_DT table = new Generate_Bar_Code_QR_Code_For_CSD_DT();
					table = (Generate_Bar_Code_QR_Code_For_CSD_DT) brandPackagingDataList
							.get(i);
	...
	Generate_Bar_Code_QR_Code_For_CSD_DT bopd;

	public Generate_Bar_Code_QR_Code_For_CSD_DT getBopd() {
		return bopd;
	}

	public void setBopd(Generate_Bar_Code_QR_Code_For_CSD_DT bopd) {
		this.bopd = bopd;
	}
	...
	public String finalizeData() {
		...
		try {
			new Generate_BarCode_QRCode_For_CSD_Impl().dataFinalize(this, bopd);
	...
	public void print() {
		...
		try {
			new Generate_BarCode_QRCode_For_CSD_Impl().generateReport(bopd, this);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void updatelisnr(ActionEvent e) {
		UIDataTable uiTable = (UIDataTable) e.getComponent().getParent()
				.getParent();
		Generate_Bar_Code_QR_Code_For_CSD_DT dt = (Generate_Bar_Code_QR_Code_For_CSD_DT) this
		...
		Generate_Bar_Code_QR_Code_For_CSD_DT dt2 = new Generate_Bar_Code_QR_Code_For_CSD_DT();
	...
	public double getDb_totalFees() {
		...
		try {
			...
			for (int i = 0; i < this.brandPackagingDataList.size(); i++) {
				Generate_Bar_Code_QR_Code_For_CSD_DT dt = (Generate_Bar_Code_QR_Code_For_CSD_DT) this.getBrandPackagingDataList().get(i);
	...
	public void calculateTotalFee(ActionEvent ae) {
		...
		this.setSum(0);
		for (int i = 0; i < this.brandPackagingDataList.size(); i++) {
			Generate_Bar_Code_QR_Code_For_CSD_DT dt = (Generate_Bar_Code_QR_Code_For_CSD_DT) this.brandPackagingDataList.get(i);
    ...
    private Generate_Bar_Code_QR_Code_For_CSD_DT cancelData;	
	...
	public Generate_Bar_Code_QR_Code_For_CSD_DT getCancelData() {
	...
	try{
			...
			int fl36_box=new Generate_BarCode_QRCode_For_CSD_Impl().checkFl36GatepassBwfl(datatable);
			int fl11_box=new Generate_BarCode_QRCode_For_CSD_Impl().checkFl11GatepassBwfl(datatable); 
	...
	public void setCancelData(Generate_Bar_Code_QR_Code_For_CSD_DT cancelData) {
...
public void cancelOrder()
	{
		...
		try{
			if(isPopupValidate())
				{
				new Generate_BarCode_QRCode_For_CSD_Impl().cancelPermit(this.cancelData, this);
					}
		}catch(Exception e)
		{
			e.printStackTrace();
	...
	//@rvind
		...
		/*public void cancelData(ActionEvent e) {
			try {
				...
				Generate_Bar_Code_QR_Code_For_CSD_DT dt = (Generate_Bar_Code_QR_Code_For_CSD_DT) this.showDataTableList1.get(uiTable.getRowIndex());     
				this.request_id=dt.getRequest_id();	
			int box=Integer.parseInt(dt.getShowDataTable_NoOfBoxes());
			
				int fl36_box=new Generate_BarCode_QRCode_For_CSD_Impl().checkFl36GatepassBwfl(dt);
				int fl11_box=new Generate_BarCode_QRCode_For_CSD_Impl().checkFl11GatepassBwfl(dt); 
