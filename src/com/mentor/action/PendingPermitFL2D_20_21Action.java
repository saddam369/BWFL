...
import com.mentor.impl.PendingPermitFL2D_20_21Impl;
import com.mentor.impl.PendingPermitImpl;

public class PendingPermitFL2D_20_21Action {

	PendingPermitFL2D_20_21Impl impl = new PendingPermitFL2D_20_21Impl();
	...
	public ArrayList getRejectionList() {
		...
		try {
			this.rejectionList = new PendingPermitImpl()
					.objectionRejectionList(this.service_id);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void viewDetails(ActionEvent e) {
		try {
			UIDataTable uiTable = (UIDataTable) e.getComponent().getParent()
					.getParent();
			PendingPermitFL2D_20_21DT dt = (PendingPermitFL2D_20_21DT) this
					.getDisplayRegUsers().get(uiTable.getRowIndex());

			this.hideDataTable = false;
			this.viewPanelFlg = true;
			this.setSt_act_flg(dt.getSt_flg());
			...
			if (this.getSt_act_flg().equalsIgnoreCase("T")) {
				this.st_total_advance_import_fees = impl
						.getst_total_advance_import_fees(this);
				this.st_total_advance_special_fee = impl
						.getst_total_advance_special_fees(this);
			} else {
	...
	public String approveApplication() {
		try {
			...
			if (this.fillRemrks != null && this.fillRemrks.length() > 0
				...
				if (this.fillValidDt.after(date1)
						|| this.fillValidDt.compareTo(date1) == 0) {
						...
						FacesContext
								...
								.addMessage(
										...
										new FacesMessage(
												...
												"Make Sure Challan Verified and Certified !!",
												"Make Sure Challan Verified and Certified !!"));
					}
				} else {
					FacesContext
							...
							.addMessage(
									...
									new FacesMessage(
											FacesMessage.SEVERITY_ERROR,
											"Validity Date Should Be greater than or equal to application date !!",
											" Validity Date Should Be greater than or equal to application date!!"));
				}
			} else {
		...
		try {
			...
			if (this.fillRemrks != null && this.fillRemrks.length() > 0) {
				...
				if (this.control_id.equals("NA")
					...
					if (!this.getObjectionRejectionCode().equals("0")
							&& !this.control_id.equals("NA")) {
						if (this.radioType.equalsIgnoreCase("N")) {
	...
	public void closeApplication() {
		...
		this.viewPanelFlg = false;
		this.setSt_act_flg("E");
		this.setSt_total_advance_import_fees(0);
		this.setSt_total_advance_special_fee(0);
	 ...
	 * AdvanceDutyRegisterBWFL_FL2D_DEOAction.setback return "GOTO";
	 * 
	 * }
	 */
	private String st_act_flg = "E";

	public String getSt_act_flg() {
		return st_act_flg;
	}

	public void setSt_act_flg(String st_act_flg) {
		this.st_act_flg = st_act_flg;
	}
	...
	public void setSt_total_advance_import_fees(
			double st_total_advance_import_fees) {
	...
	public void setSt_total_advance_special_fee(
			double st_total_advance_special_fee) {
