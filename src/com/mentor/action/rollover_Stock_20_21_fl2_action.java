...
import com.mentor.impl.rollover_Stock_20_21_fl2_impl;



public class rollover_Stock_20_21_fl2_action {
	rollover_Stock_20_21_fl2_impl impl=new rollover_Stock_20_21_fl2_impl();
...
//-----------------------------Delete methods --------Created by Arvind-Verma----------------------------------------------------------		  
	...
	public String finalizeData()
	{
		...
		try{
			...
			if(!new rollover_Stock_20_21_fl2_impl().validatechallan(this,prt)){
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("Challan  Not Found ! ", "Challan  Not Found!"));	
			}else
			{
			boolean flag=new rollover_Stock_20_21_fl2_impl().updatechallandetail(this,dt);
			if(flag)
			{
