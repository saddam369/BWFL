...
import com.mentor.impl.GatepassToDistrictBWFLImpl;
import com.mentor.utility.Validate;

public class GatepassToDistrictBWFLAction {

	GatepassToDistrictBWFLImpl impl = new GatepassToDistrictBWFLImpl();
	...
	private double adv_consideration_fee;
	private double adv_scanning_fee, adv_duty;
	private boolean save_flag;
	private String msg="";

	public double getAdv_duty() {
		return adv_duty;
	}

	public void setAdv_duty(double adv_duty) {
		this.adv_duty = adv_duty;
	}
	...
	public double getAdv_consideration_fee() {
		return adv_consideration_fee;
	}

	public void setAdv_consideration_fee(double adv_consideration_fee) {
		this.adv_consideration_fee = adv_consideration_fee;
	}

	public double getAdv_scanning_fee() {
		return adv_scanning_fee;
	}

	public void setAdv_scanning_fee(double adv_scanning_fee) {
		this.adv_scanning_fee = adv_scanning_fee;
	}
	...
	if(dt1.isSelected())
	{
		...
		for (int i = 0; i < this.displaylist.size(); i++) {
			GatepassToDistrictBWFLDT dt = (GatepassToDistrictBWFLDT) displaylist
					.get(i);
			...
			if (isValidateInput1()) 
			{
				...
				impl.saveMethodImpl(this);
						...
						.addMessage(
								...
								new FacesMessage(
										FacesMessage.SEVERITY_ERROR,
										" First clear/Delete all your Draft Gatapasses !!",
										" First clear/Delete all your Draft Gatapasses !!"));
					}
				}
	...
	public void printReport(ActionEvent e) {
		...
		GatepassToDistrictBWFLDT dt = (GatepassToDistrictBWFLDT) this.table2List.get(uiTable.getRowIndex());
	...
	public double getDb_total_value() {
		...
		try {
			...
			for (int i = 0; i < this.displaylist.size(); i++) {
				GatepassToDistrictBWFLDT dt = (GatepassToDistrictBWFLDT) this
	...
	public void calculateTotalDuty(ActionEvent ae) {
		...
		if (isFlag()) {
			this.setSum(0);
			for (int i = 0; i < this.displaylist.size(); i++) {
				GatepassToDistrictBWFLDT dt = (GatepassToDistrictBWFLDT) this.displaylist
						.get(i);
	...
	public void scanAndUpload(ActionEvent ae) {
		UIDataTable uiTable = (UIDataTable) ae.getComponent().getParent()
				.getParent();
		GatepassToDistrictBWFLDT dt = (GatepassToDistrictBWFLDT) this.table2List
	...
	private GatepassToDistrictBWFLDT edidata;

	public GatepassToDistrictBWFLDT getEdidata() {
		return edidata;
	}

	public void setEdidata(GatepassToDistrictBWFLDT edidata) {
...
public void calculateTotalCeshDuty(ActionEvent ae) {
	...
	if (isCeshflag()) {
		this.setCeshsum(0);
		for (int i = 0; i < this.displaylist.size(); i++) {
			GatepassToDistrictBWFLDT dt = (GatepassToDistrictBWFLDT) this.displaylist.get(i);
