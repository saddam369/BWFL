...
import org.jbpm.context.exe.converter.DoubleToStringConverter;
import org.richfaces.component.UIDataTable;
 
import com.mentor.datatable.Import_Permit_FL2D_20_21DT;
import com.mentor.impl.Import_Permit_FL2D_20_21Impl;
import com.mentor.utility.Utility;
import com.mentor.utility.Validate;

public class Import_Permit_FL2D_20_21Action {

	Import_Permit_FL2D_20_21Impl impl = new Import_Permit_FL2D_20_21Impl();
	...
	public void deleteRowMethodPackaging(ActionEvent e) {
		UIDataTable uiTable = (UIDataTable) e.getComponent().getParent()
				.getParent();
		Import_Permit_FL2D_20_21DT dt = (Import_Permit_FL2D_20_21DT) this.displaylist.get(uiTable
				.getRowIndex());
	...
	public String getTotal_import_fees() {
		...
		try {
			for (int i = 0; i < this.displaylist.size(); i++) {
				Import_Permit_FL2D_20_21DT dt = (Import_Permit_FL2D_20_21DT) this.getDisplaylist().get(
						i);
	...
	public String getTotal_special_fee() {
		...
		try {
			for (int i = 0; i < this.displaylist.size(); i++) {
				Import_Permit_FL2D_20_21DT dt = (Import_Permit_FL2D_20_21DT) this.getDisplaylist().get(i);
	...
	public void reset() {
		...
		this.importChallanList.clear();
		this.spclChallanList.clear();
		this.custom_unit_list.clear();
		this.importer_unit_id=0;
		this.consignor_nm_adrs=null;
		impl.ChallanList(this);
	}
		...
		if (validateInput) {
			if (this.displaylist.size() > 0) {
				for (int i = 0; i < displaylist.size(); i++) {
					...
					Import_Permit_FL2D_20_21DT table = new Import_Permit_FL2D_20_21DT();
					table = (Import_Permit_FL2D_20_21DT) displaylist.get(i);
	...
	public void save() {
		try {
			if (isValidateInput()) {
				...
				//System.out.println("====getSt_total_advance_import_fees======"+this.getSt_total_advance_import_fees()+"========"+Double.parseDouble(this.getTotal_import_fees()));
				 //System.out.println("====getSt_total_advance_special_fee======="+this.getSt_total_advance_special_fee()+"============"+Double.parseDouble(this.getTotal_special_fee()));
				if(this.getSt_total_advance_import_fees()>= Double.parseDouble(this.getTotal_import_fees())){
				 	 if(this.getSt_total_advance_special_fee() >= Double.parseDouble(this.getTotal_special_fee())){
				if(impl.save(this)){
					this.setPanel("H");
					this.flag = "F";
					this.hislist = impl.hislistImpl(this);
				//}
						...
						FacesContext.getCurrentInstance().addMessage(null,new FacesMessage (FacesMessage.SEVERITY_ERROR,
								"Total Special Fee should not greater than Total Advance Special Fee!!!!!!!!" ,
								"Total Special Fee should not greater than Total Advance Special Fee!!!!!!!!"));
					}
			}else{
				FacesContext.getCurrentInstance().addMessage(null,new FacesMessage (FacesMessage.SEVERITY_ERROR,
						"Total Import Fee and Duty  should not greater than Total Advance Import Fee!!!!!!!!" ,
						"Total Import Fee and Duty  should not greater than Total Advance Import Fee!!!!!!!!"));
			}
			} else {
	...
	public void view(ActionEvent e) {
		UIDataTable uiTable = (UIDataTable) e.getComponent().getParent()
				.getParent();
		Import_Permit_FL2D_20_21DT dt = (Import_Permit_FL2D_20_21DT) this.hislist.get(uiTable
				.getRowIndex());
		this.flag = "T";
		this.setApp_id(dt.getApp_id());
		this.brand_pack_list = impl.viewdetailImpl(this, dt.getApp_id(),
	...
	public void deletelisnr(ActionEvent e) {
		...
		Import_Permit_FL2D_20_21DT dt = (Import_Permit_FL2D_20_21DT) this.hislist.get(uiTable
				.getRowIndex());
		 try {
			impl.delete(this, dt.getApp_id(),dt.getDt_district_id(), dt.getApp_date(), dt.getImport_fee_challan_no());
	...
	//rahul 21-09-2020 for total advance import and special fess
	private double st_total_advance_import_fees;
	private double st_total_advance_special_fee;


 
	public double getSt_total_advance_import_fees() {
		double total_advance_importFees=0.0;
		double total_advance_specialFees=0.0;
		 try{
				
					
					total_advance_importFees=impl.getst_total_advance_import_fees(this);
					total_advance_specialFees=impl.getst_total_advance_special_fees(this);
					DecimalFormat formatter = new DecimalFormat("#.##");
					this.st_total_advance_import_fees = total_advance_importFees;
					this.st_total_advance_special_fee=total_advance_specialFees;
					
				
		 }catch(Exception e){
			e.printStackTrace(); 
		 }
		return st_total_advance_import_fees;
	}

	public void setSt_total_advance_import_fees(double st_total_advance_import_fees) {
		this.st_total_advance_import_fees = st_total_advance_import_fees;
	}

	public double getSt_total_advance_special_fee() {
		
	 
		return st_total_advance_special_fee;
	}

	public void setSt_total_advance_special_fee(double st_total_advance_special_fee) {
		this.st_total_advance_special_fee = st_total_advance_special_fee;
	}
