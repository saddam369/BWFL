...
import com.mentor.impl.ImportPermitImpl;
import com.mentor.utility.Utility;
import com.mentor.utility.Validate;

public class ImportPermitAction {

	ImportPermitImpl impl = new ImportPermitImpl();
	...
	public void deleteRowMethodPackaging(ActionEvent e) {
		UIDataTable uiTable = (UIDataTable) e.getComponent().getParent()
				.getParent();
		ImportPermitDT dt = (ImportPermitDT) this.displaylist.get(uiTable
				.getRowIndex());
	...
	public String getTotal_import_fees() {
		...
		try {
			for (int i = 0; i < this.displaylist.size(); i++) {
				ImportPermitDT dt = (ImportPermitDT) this.getDisplaylist().get(
						i);
	...
	public String getTotal_special_fee() {
		...
		try {
			for (int i = 0; i < this.displaylist.size(); i++) {
				ImportPermitDT dt = (ImportPermitDT) this.getDisplaylist().get(i);
...
System.out.println("--"+this.route_detail+"--");
		if (validateInput) {
			if (this.displaylist.size() > 0) {
				for (int i = 0; i < displaylist.size(); i++) {
					ImportPermitDT table = new ImportPermitDT();
					table = (ImportPermitDT) displaylist.get(i);
	...
	public void view(ActionEvent e) {
		UIDataTable uiTable = (UIDataTable) e.getComponent().getParent()
				.getParent();
		ImportPermitDT dt = (ImportPermitDT) this.hislist.get(uiTable
				.getRowIndex());
		this.flag = "T";
	...
	public ArrayList getImporter_unit_list() {
		System.out.println("login type="+this.getVch_license_type());
